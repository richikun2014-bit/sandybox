...
### Fixed
- fixed VMware Workstation Pro 26H1 QueryDosDevice crash [#5390]
...
### Changed
- improved box import/export behaviour [#5362]
...
### Fixed
- fixed Epic Games not launching [#5281] [#5303] [#5344]
...
### Changed
- hardware info protection can now be switched per process [#5271]
- driver now verifies certificates using UTC time instead of local time [#4241] [#5316]

### Fixed
- fixed issue with volatile configuration update
- fixed, added missing WaitServiceState, resolves "LINE fails to launch" / NO_SIGNATURE [#5306]
- fixed release Ipc_Handles_CritSec on Ipc_GetName failure in NtQueryDirectoryObject [#5326]
- fixed the user interfaces of all WebView2 applications built with Tauri are unresponsive [#5327]
- fixed race condition BSoD during driver unload [#5329]
- fixed Key_MergeSubkeys logic verification [#5301]
...
### Added
- added configurable window location settings to control which monitor main, non-main, file recovery, and notification windows open on, including a selectable fallback mode [#4536] [#5238]
  - Global Settings > Interface Config > Window Options
- added "Label only" border mode option (`onlbl`, `ttllbl`, `alllbl`) that hides the colored border frame and shows only the sandbox name (or alias) label [#5239]
- added MIDI workaround template for Windows 11 [#5183] [#5203] (thanks xsmolasses)
- added new tray customization options (Global Settings > Shell Integration > System Tray): [#5254]
...
- fixed double-clicking on a group's empty path/command line crash [#5253]
- fixed compact tray box list clipping long sandbox names; width is now measured precisely per item using font metrics and scales correctly at any DPI [#5254]
- fixed WOW64 registry view inheritance for relative key opens in `SbieDll`, preserving parent `KEY_WOW64_32KEY/KEY_WOW64_64KEY` semantics across `NtOpenKey`/`NtCreateKey` [#5171] [#5244]
...
- fixed parsing logic for `ClosedClsid` and `ClosedRT` settings [#5263]
...
- FIXED SECURITY ISSUE ID-34: Sandybox EditAdminOnly Bypass via INI CRLF Injection CVE-2026-34458 (reported by sammy12342)
...
- reworked `Rename Sandbox` dialog in SandMan: added dedicated alias controls, persisted `Hide alias` preference, improved stable/dynamic dialog resizing, and disabled alias is now saved as `BoxAliasDisabled` [#4657] [#5231]

### Fixed
- fixed border overlay getting stuck when border label text is disabled (`no` mode) [#5230]
...
- fixed duplicate Name column appearing in the Config Dump tab under Template Settings [#4900]  [#5232]
...
### Fixed
- fixed regression SbieSvc IPC failing on Windows 10 introduced in 1.17.0 [#5226]
...
### Added
- added icons to Sandybox service applications [#5160]
...
- fixed File Searching bar crashes when performing multiple searches within a sandbox [#5166] [#5221]
...
### Added
- added Norwegian Bokmål translations [#5141] (thanks divinity76)
- added user interface: highlight last used box [#5054]
...
- fixed WinMerge keeps freezing because of registry reading issues [#5122]
- fixed running Firefox under Sandybox causes AppModel-Runtime error(s) in Event Viewer [#5135]
- fixed Tor Browser tabs crash in highest security level [#5116]
- fixed Chrome Portable window tooltip gets stuck [#5051]
- fixed BSoD (0x50) in SbieDrv.sys during high-load GPU process creation on Windows Server 2022 [#5149]
- fixed OfficeClickToRun.exe always crash, WINWORD.EXE cannot work properly [#5153]
- fixed prolonged lag when uninstalling/deleting software installed exclusively within the sandbox environment [#5028]
...
- preserve custom box colors: custom colors set for a sandbox will now persist even after changing the `Box Type Preset` [#5025] (thanks Pinefone)
- added alpha transparency to sandbox border settings [#5037]
- user interface: start with all groups collapsed [#5052]
- user interface: add menu option to collapse/expand groups [#5053]

### Changed
- support certificates can now contain multiple HwID's to simplify handling
- context menu icon upgrade [#5038]
...
### Fixed
- fixed BindAdapter reliability when VPN/bound adapter is disconnected [#5055] [#4896]
- fixed WOW6432Node bug, becomes `HKLM\SOFTWARE\WOW6432Node\WOW6432Node` upon necessary creation of a 2nd non-existent subkey [#5057]
- fixed PowerShell is able to launch an executable inside a folder added to program control [#5027]
...
### Fixed
- fixed Sandybox 1.16.3 fails to connect to driver on Windows Preview build 27943 [#5003]
...
### Changed
- reworked driver imports to despite of being build with VS2022 allow it to load on Windows 7 [#4942]
...
### Fixed
- fixed 1.16.2 = tray menu missing in action [#4940]
- fixed UAC prompt layout at higher DPI in secure desktop [#4953] (thanks gexgd0419)
- fixed RTL layout for Arabic in UAC prompt dialog [#4957] (thanks gexgd0419)
- fixed 1.16.1 dark mode help popup window with light color background [#4903]
- fixed network options cannot be persisted during editing [#4899]
- fixed cleanup on aisle Qt5 - installer should remove obsolete files after Sandybox 1.16.x [#4934]
...
### Added
- added toggleable INI key validation to "Edit ini Section" [#4915] (thanks offhub)
  - added toggleable per-key tooltip support [#4928] (thanks offhub)
- added option to use the new Qt Windows 11 style on SandMan [#4927] (thanks LumitoLuma)

### Changed
- ImBox no longer updates container file timestamps when accessing an encrypted box volume

### Fixed
- fixed Windows 11 24H2 build 26100.4770 causes Firefox Portable 140.0.4 / 141.0 to stop responding upon starting it sandboxed [#4920]
...
### Added
- added UI Font in settings windows [#4894] (thanks srdr0p)
...
### Fixed
- fixed incomplete directory in links when creating shortcuts [#4882]
- fixed BindAdapter doesn't work, can still access internet when bound adapter (VPN) is disabled [#4896]
- fixed there's a strange problem when calling GetFullPathName() with a string starting with a (back)slash [#4867]
- fixed some of the UI bound with the install language ID [#4859]
- fixed bad UAC prompt layout [#4863]
- fixed "Run Un-Sandboxed" option ignored when Mark of The Web forcing is enabled [#4856]
...
- added wildcard support with ForceProcess [#2603]
- added support for proper text scaling in the 'Type' dropdown on the Trace Log page to prevent truncation when display scaling is applied [#4812] (thanks wzxjohn)
...
- added option 'UseSandboxieUAC=y' to use Sandybox's own UAC stand-in prompt instead of the system default one
  - Note: the new UAC prompt also offers the option to grant only fake admin privileges to the application

### Changed
- refactored **DNS Filter logic** to enable filtering even if no DNS resolution occurs or no valid IP address is returned [#4811] (thanks wzxjohn)
...
- improved Move Sandbox menu [#4819]

### Fixed
- fixed Sandybox taskbar remaining visible despite entering full-screen mode in Firefox 138+ [#4761]
- fixed redundant options when deleting multiple boxes [#4817]
- fixed Sandybox closing unexpectedly
- fixed page display abnormality [#4810]
- fixed working directory when running program from context menu on Windows 11 [#4844] (thanks micbou)
...
### Added
- added Indonesian translation to the Plus UI [4bad4d1] (thanks ???)
- added template for WeType Input Method (IME) [#4715] (thanks bot-1450)
- added host name support to proxy configuration (looking up host at WSA_Startup)
- instead of binding to IP, Sandybox can now bind to adapter (resolving IPs at WSA_Startup)
- added support for %ProgramFiles(x86)% variable [#2402]
- added TemplateDefaultFolders containing all the default paths to be created in a privacy enhanced box

### Changed
- changed sorting method for nested sandbox groups in the Move Sandbox list [#4615]
- name and HwID information in the Support tab are now hidden by default but can be toggled as needed [#4714]
- added alternative proxy relay implementation (instead of hacking existing sockets it starts relay threads) [EXPERIMENTAL]

### Fixed
- fixed BoxAlias missing from the 'run in box' selection prompt [#4709]
- fixed Ctrl+F collapsing all expanded box groups [#4708]
- FIXED SECURITY ISSUE ID-26: added security checks to registry set/get functions CVE-2025-46713, CVE-2025-46714, CVE-2025-46716, CVE-2025-46715 [5a6a2f7] (thanks sgayou)
- fixed issue in Pool_Alloc [5a6a2f7]
- fixed SbieCtrl.exe and SandMan.exe causing a constant 3 MB/s IO rate [#4693]
- fixed Tencent TIM template preventing drag and drop maneuvers [#4688]
- improved responsiveness for huge Sandboxie.inis further [#4573]
- fixed missing crucial AppData\Roaming\Microsoft folder in Data Protection Box [#4711]
- fixed time speeding errors in Sandybox and added time function hooks [#4721] (thanks pwnmelife)
...
- fixed toolbar language obeying UI language changes [#4726]
- fixed PingInfoView compatibility [#4718]
- fixed CollectProducts() looping if no uninstall key is found [#4753]
- fixed crashes with SbieDll.dll [#4754]
- fixed OneDrive triggering downloads when creating shortcuts [#4663] [4750] (thanks WZ-Tong)

### Removed
- removed the not-working "delete content" button [#4720] (thanks habatake)
...
### Fixed
- fixed Firefox 137 throwing SBIE2328 errors in Sandybox v1.15.9 [#4652] [#4652] [#4640]
- fixed SbieShellExt.dll missing in 1.15.9
- fixed Process_GetCommandLine causing memory leak [#4658]
- fixed Thunderbird 102.15.1 add-on blockage in Sandybox v1.15.9 [#4653]
- fixed invoking SetParent change window to message-only window [#4656]
- fixed issue with Unicode names when retrieving an evaluation certificate
- fixed delete content message box to always appear on top [#4673]
...
### Added
- added new sandbox duplication function that duplicates entire sandboxes including files [#4542]
- added new insecure debug option 'OpenAllSysCalls=y'
- added template for WindowGrid
- added new certificate type [DEVELOPER](https://xanasoft.com/product/Sandybox-developer/); it deactivates the user mode component verification, but can only be used in node-locked form
...
- added warning if a sandbox is put in the root of a partition [#4582]
...
### Fixed
- fixed text encoding issue in the box delete/remove confirmation dialog [#4591]
- fixed duplication of template settings if the the box settings/type is changed [#4595]
- fixed bug when uninstalling [#4540]
- fixed Golang Cmd.Run() doesn't return when running in Sandybox [#4538]
  - fixed a generic handle duplication which may help many other applications as well
- improved responsiveness for huge Sandboxie.inis [#4573]
- fixed Sandybox freezes/crashes in certain installations [#4537]
...
- fixed hooking issues introduced in Firefox 137 Nightly [#4522]
- fixed issue with insecure debug option 'OriginalToken=y'
- fixed box group name accepting line breaks [#4608]
...
### Changed
- added 'Creation Time' column to snapshot manager (thanks PenXLa) [#4555]
- improved sandbox deletion confirmation prompt (thanks PenXLa) [#4561]
- updated online updater
- added [ImDisk 3.0](https://github.com/DavidXanatos/ImDisk/releases/tag/v3.0.0) driver to Sandybox installer (x64 only)

### Fixed
- fixed "WriteFilePath" not working correctly [#4553]
- fixed an issue where Box Notes deleted matching lines when saved [#4547]
...
### Changed
- updated Notepad++ template to work in Sandybox Classic
- updated MiscHelpers.dll, improved finder

### Fixed
- fixed obsolete entries in the Sandybox installer
- fixed dysfunctional shortcuts created by 'Run from Start Menu' [#4502]
- fixed potential BSoD issue with driver in low resource conditions
- fixed a crash in NsiAllocateAndGetTable (thanks tzf-omkey) [#4511]
- fixed a memory leak when device change notification is received by a sandboxed process
- fixed a memory leak in NtQueryDirectoryFile hook occurring in privacy enhanced boxes [#4509]
...
### Fixed
- fixed Sandybox data folder issues after updating to version 1.15.5
- fixed prolonged locks in the troubleshooting engine
- fixed Adobe Acrobat Reader crashes in compartment boxes [#4363]
- fixed Viber crashes [#4298]
- fixed qutebrowser compatibility with Sandybox [#4419]
- fixed ApiTrace preventing program launches in version 1.15.5 [#4488]
...
### Changed
- moved Sandybox data folder from %LOCALAPPDATA%\\Sandybox to %LOCALAPPDATA%\\Xanasoft\\Sandybox
  - Note: the folder will be moved automatically. When downgrading to an older version it needs to be moved back manually.
- re-ordered box context menu options [#4444]
...
- fixed program icon not displaying if the command path in RunCommand setting contains spaces [#4416]
- fixed template scanning not working properly [#4401]
- fixed credentials dialog not displaying when connecting to RDP server by mstsc.exe in encrypted box [#4389]
- fixed folder recovery only moving files inside that folder [#4453]
- fixed failure to query file properties causing program exceptions [#4457] (thanks lmou523)
- fixed applications crashing if IP is not defined for the domain name in DNS Filtering [#4475]
- fixed files not getting written when launched from "Run Program" menu [#4398]
...
### Added
- added template for Joplin [#4402] (thanks offhub)

### Fixed
- fixed dark mode for the INI editor
- fixed deleting sandbox content [#4407] (thanks bot-1450)
- fixed "run unsandboxed" no longer working from box picker window [#4403]
- fixed Firefox tab crashes when running with `ProtectHostImages=y` enabled [#4394]
...
- fixed BSoD "SYSTEM_SERVICE_EXCEPTION (3b)" when opening a DLL from AlertFolder using x64dbg [#4421]
- fixed BSoD "CRITICAL_PROCESS_DIED" when terminating all sandboxed programs [#1316]
  - Note: we now terminate boxed processes individually instead of using the job object, unless "TerminateJobObject=y" is set
- fixed INI editor font selection not working after addition of the highlighting feature [#4429]
...
- added custom font configuration for Sandybox UI [#4397] (thanks habatake)

### Changed
- improved support notification
- improved deletion of Sandybox leftovers [#4374]
...
- updated to Qt 5.15.16 with OpenSSL 3.4.0 [#4370] (thanks offhub)

### Fixed
- fixed .tmp file signing during Sandybox installation or update [#2643] [#4343]
- fixed issue with DLL unloading
- fixed "Files Resource Access" granting access to excluded folders [#4007]
- fixed "Run Outside Sandbox" disappearing from the run menu if "ForceDisableAdminOnly=y" is set [#4233]
- fixed deadlock on no-op condition when renaming file or folder [#4304]
- fixed moving sandboxed files or folders [#4329]
- fixed Sandybox causing Firefox Nightly crashes [#4183]
...
- updated to Inno Setup 6.3.3 [#4020]

### Fixed
- fixed Sandybox crypto fails to start in red boxes
...
- updated to Qt 5.15.15 with OpenSSL 3.3.2 [#4223] (thanks offhub)
...
### Fixed
- fixed exported encrypted archive files cannot be unpacked by Sandybox [#4229]
...
### Changed
- allow users to import/export boxes with .zip files [#4200]
...
- fixed NtQueryDirectoryObject (should not return "STATUS_MORE_ENTRIES") as this is an easy sandbox detection [#4201]
...
### Added
- added alias for sandboxes [#4112]

### Fixed
- fixed CustomLCID issue under Windows 7 [#4117]
...
### Added
- added HwID display
- added language spoofing "CustomLCID=1033" [#4024] (thanks Yeyixiao)
...
### Fixed
- fixed multiple supporter certificate pop-ups if sandbox settings are opened [#4074]
...
- fixed memory limit for job objects, which can now exceed 4GB [#4096]
...
### Changed
- improved removal of leftovers [#4050]

### Fixed
- fixed the Start Restrictions tab layout breaking [#4045]
...
### Changed
- updated to Qt 5.15.14 with OpenSSL 3.3.1 [#3994] (thanks offhub)

### Fixed
- fixed applications unable to launch as admin in a sandbox with "UseCreateToken/SandboxieAllGroup" when using an MSFT account [#4022]
...
- fixed Sandybox programs not terminating after closing programs that run as admin with UseCreateToken/SandboxieAllGroup enabled [#4030]
...
### Added
- added SbieIni option to modify password-protected configurations [#3903]
...
- added option "HideSbieProcesses" to hide Sandybox Work Process (SbieSvc, SandboxieRpcSs etc.)
...
- fixed export sandbox function not including hidden files [#3980] (thanks L4cache)
- fixed Chrome printer stoppage [#3926]
  - Sandybox will add CustomChromiumFlags=--disable-features=PrintCompositorLPAC to Chromium-based browsers command line
  - Note: Less Privileged App Container (LPAC) aren't working with Sandybox presently
- fixed accessing relative symlinks which refer to a target that starts with a dot [#3981]
- fixed opening a sandbox's properties window via double-click in the systray context window [#3861]
...
- fixed compatibility issue with FakeAdminRights [#3989] (thanks offhub)
...
- added option "AlertBeforeStart"
  - when it is set, a prompt pops up before launching a new program into the sandbox using "Start.exe" and checks if the program that started "Start.exe" is a Sandybox component itself, if it is not, a warning pops up
...
### Added
- added file version information for SbieDll.dll and SbieSvc.exe in the Sandybox about dialog

### Changed
- improved checkboxes about DropAdminRights in SandMan [#3851] (thanks offhub)

### Fixed
- fixed symbolic linking of files [#3852]
...
- added editable trigger list [#3742]
- added optional extension of the screenshot protection to the UI [#3739]
- added a button to edit local/custom templates [#3738]
- added adjustable resizing of the "Run Sandboxed" window [#3697]
- added Notepad++ template [#3836] (thanks offhub)

### Changed
- improved Avast template [#3824] (thanks offhub)
...
### Fixed
- fixed inactive apply button when changing BlockDNS or BlockPorts options [#3807]
- fixed troubleshooting wizard breaking with new Qt [#3810]
- fixed Settings dialog now showing the correct RAM drive letter
- fixed broken updater due to missing SSL support in the latest Qt build [#3810]
...
- fixed Sandybox 1.13.4 with IsBlockCapture=y not working on Windows 7 [#3769]
- fixed explorer.exe issue "FakeAdminRights=y" [#3638]
- fixed forced folder warning notification [#3569]
...
### Added
- added ability to set all processes to a certain core per box [#3276]
...
### Changed
- on systems in test signing mode, Sandybox will try outdated offsets by default
- updated to Qt 5.15.13 with latest security patches [#3694] (thanks LumitoLuma)
...
- improved MPC-BE template [#3798] (thanks offhub)

### Fixed
 - fixed Virtualization scheme Version 2 causing extremely slow file deletion speeds [#3650]
...
- added a menu and button/icon to suspend all processes [#3741] ()

### Changed
- option "LingerLeniency=n" now also disables the 5 second grace period for freshly started lingerers [#1892]
...
- reworked offset dependent handling of undocumented Windows kernel objects
  ...
  - when Sandybox encounters a yet unsupported kernel build, token based isolation is disabled to prevent system instability
...
- enabled CET Shadow Stack compatible flag for core Sandybox binaries
...
### Fixed
- fixed issue with symlink resolver introduced in the previous build [#3481]
...
### Fixed
- fixed warning issue: `SBIE2321 Cannot manage device map: C0000034 / 11` [#2996]
- fixed issue with explorer.exe that could not run on emulate admin sandbox [#3516]
- fixed potential BSoD issue with WFP when trace logging is enabled [#2471]
- fixed issue with running programs pinned to the Run menu that did not use the same working directory [#3555]
- fixed UseNewSymlinkResolver causing applications to create both the link and the target folder [#3481]
- fixed Plus UI notification window becoming stuck when a modal dialog is displayed [#3586]
...
### Fixed
- fixed "Duplicate Box Config" option not being able to keep the order of the original sandbox configuration [#3523]
- fixed "Save options as new defaults" option not appearing when "Configure Advanced Options" is enabled [#3522]
- fixed Windows Explorer issue when running in a sandbox with data protection enabled on Windows 11 [#3517]
  - the default template for privacy boxes now sets `NormalFilePath=%ProgramData%\Microsoft\*`
- fixed message boxes with MB_DEFAULT_DESKTOP_ONLY or MB_SERVICE_NOTIFICATION not being able to display title and text correctly in Security Hardened sandboxes [#3529]
- fixed issue with npm failing to run inside a Security Hardened sandbox on Windows 11 [#3505]
- fixed symlink and open path issue introduced in 1.12.6 [#3537]
  - when a volume without an associated drive letter is encountered, Sandybox uses `\drive\{guid}` instead of `\drive\[letter]`
  - Note: if the volume is subsequently assigned a drive letter, the data under `\drive\{guid}` will be ignored!
- fixed "Run Unsandboxed" command when right-clicking shortcuts created with Sandybox [#3528]
...
- fixed Privacy Enhanced sandboxes failing with error code SBIE2204 [#3542]
- fixed OpenFilePath directories not being enumerated within their parent folders [#3519]
...
### Fixed
- fixed issue introduced in 1.12.4 with Start.exe failing to run in an encrypted box [#3514]
...
### Fixed
- fixed bug with Chromium-based browsers affecting "--type=crashpad-handler" instances [#post-3177139](https://www.wilderssecurity.com/threads/Sandybox-1-12-5.453131/#post-3177139)
...
### Added
- added the display of date and time in the Sbie Messages tab [#3429]
...
- the clean-up button in SandMan UI is now enabled even when not connected to the Sandybox core components
- the box creation wizard now allows to create encrypted sandboxes based on any other box type

### Fixed
- fixed running sandboxed processes located in an ImDisk volume [#3472]
- fixed sample 634d066fd4f9a8b201a3ddf346e880be unable to be terminated on Windows 7 x64 [#3482]
- fixed UseNewSymlinkResolver causing applications to create both the link and the target folder [#3481]
- fixed issue with sandbox renaming that broke the group hierarchy [#3430]
- fixed encrypted sandbox with red box preset blocking access to its root directories [#3475]
- fixed SandMan crash issue introduced in 1.12.0 [#3492]
...
### Fixed
- fixed issue with Microsoft Edge when using AutoDelete option [#post-3173507](https://www.wilderssecurity.com/threads/Sandybox-v1-12-1-pre-release.452939/#post-3173507)
- fixed warning issue with Acrobat Reader: `SBIE2205 Service not implemented: CredEnumerateA` [#issuecomment-1826280016]
...
- fixed online updater not respecting portable mode [#3406]
- fixed snapshot feature not working on encrypted boxes [#3439]
...
- fixed warning issue with Firefox 120.0: `SBIE2205 Service not implemented: CredReadA` [#3453]
...
- added troubleshooting script for parental controls [#3318]
...
- added customizable global hotkey that toggles "pause forced programs" [#2441]
- added warning to prevent broad "forced folder" settings [#650]
- added CheckExpectFile function to Sandybox [#768]
...
- fixed typo: 'NoRestartOnPAC' option becomes 'NoRestartOnPCA' [0e175ee]
...
- fixed some GUI functions using the wrong box path if the FileRootPath value starts with Volume{GUID} [#3345]
- fixed programs not running when they are kept in RAM [#3349]
- fixed Process Restrictions toggle not working correctly [#3355]
...
### Added
- added container header backup/restore options to the Sandbox Options window [#post-3165085](https://www.wilderssecurity.com/threads/Sandybox-v1-11-3.452487/#post-3165085)

### Changed
- updated to 7-Zip 23.01 [4ee1464]
...
- added an option to terminate the UI by clicking the close button [#3253]
- added an option to restore a Sandybox window which has been minimized to the system tray with a single click [#3251]
- added an option to minimize Sandybox to the system tray by clicking minimize button [#3252]
- added an independent desktop integration setting [#3246]
- added template preset for AdGuard Browser Assistant [#3283]

### Changed
- moved tray options to their own tab
- replaced some graphics with better compressed ones (thanks idealths) [1af82bb]
- changed the /mount_protected command line switch to be checked before the /mount command line switch [#3289]

### Fixed
- fixed subscription certificate recognition issue
- fixed logo cut-off in the About window [#3249]
- fixed issue with file recovery when using a RAM drive [d82b62e]
...
### Added
- added 'RamDiskLetter=R:\' option allowing to mount the RAM drive root to a drive letter [938e0a8]

### Changed
- changed the new option layout to be the default for non-vintage views (can be changed back in the settings) [94c3f5e]

### Fixed
- fixed issue when re-creating a RAM sandbox junction [2542351]
- fixed Sandybox logo scaling during the setup wizard [#3227]
- fixed text cut-off in box creation wizard [#3226]
- fixed Windows 7 compatibility issue with ImBox.exe [1f0b2b7]
- fixed a bug where InjectDll/InjectDll64 settings were not correctly added to the sandbox configuration [#3254]
- fixed issue with 'UseNewSymlinkResolver=y' [01a2670]
- fixed SandMan crash in Vintage View mode [#3264]
- fixed pinning of more than one shortcut that points to the same file [#3259]
...
### Changed
- submenus don't close on mouse-leave (thanks typpos) [#2963]
...
### Fixed
- fixed Sandybox-x64-v1.10.3 crash on startup [#3174]
- fixed issue with untranslated buttons in all wizards [#3133]
...
- Add-Ons Manager: added tooltip to version column with maintainer information [#3167]
- added mechanism to open websites for add-ons [#3166]
...
- fixed issues with Qt-based installers [#2493] [#3153]
...
### Changed
- changed format of the add-on data [#3135]
  - all users coming from versions 1.10.0 and 1.10.1 will need to reinstall the components in the Add-On Manager
- "OpenClipboard=n" now also denies writes to clipboard [#1367]

### Fixed
- fixed cross-renaming of directories
- fixed auto-scroll not working [#393]
- fixed UI issue with box removal protection [#3104]
...
### Added
- added UI option to select a custom text editor to open Sandboxie.ini [#3116]
- added separate protection against box removal and content deletion [#3104]
- added "auto scroll" feature in the Trace Log tab [#393]
...
- changed Delete V2 scheme to use drive letters in FilePaths.dat (backwards-compatibility with NT paths remains) [#3053]
- improved robustness of snapshot merging [#3017]

### Fixed
- fixed "Disable Security Isolation" causing a game to stop playing audio [#2893]
- fixed NtQueryDirectoryObject not implemented [#2734]
- fixed issue with working directory for Run Menu entries
- fixed improper global symlink in sandboxed namespace [#3112]
...
- fixed asynchronously assigned PCA job not being properly detected [#1919]
- fixed incompatibility with first Windows 10 release [#3117]
- fixed remove sandbox only deletes the contents of the sandbox when an application is running in the sandbox [#3118]
- fixed crash issue with the script engine not terminating properly [#3120]
- fixed ImDisk under Sandybox supervision causes SBIE2337 and sometimes BSoD [#1092]
- fixed snapshots not merging duplicate directory junctions [#3016]
...
### Removed
- removed template support for Internet Download Manager [#991]
...
- added `What's new in Sandybox` dialog in SbieCtrl.exe to highlight the new features of the Plus UI
  - Note: this is shown after the installation of Sandybox Classic
- added "fixdacls" command to KmdUtil.exe, it repairs broken DACL entries on the Sandybox folder to fix issues where SbieDll.dll fails to load
- added option to hide Sandybox's own processes [#3054]
- added functionality to cache Sandybox messages within the Plus UI [#2920]
...
- split the support page into Sandybox Support and Sandybox Updater tabs
...
### Fixed
- fixed uninstall issue in the Sandybox Classic installer [d1863ff]
...
### Changed
- improved uninstall process in the Sandybox Classic installer (thanks sredna) [#3044]

### Fixed
- fixed a link error when the path was created with a symbolic link (thanks lmou523) [#3038]
- fixed recovery window focus after file overwrite dialog [#3048]
- fixed Sandybox crash with exception code c0000005 [#3024]
...
- fixed Plus installer not honouring IniPath value when updating the current installed version (thanks offhub) [#2994]
- fixed timestamp on crash dumps
- fixed Firefox 114 crashes in Security Hardened boxes, if start restrictions are enabled (thanks offhub) [#3019]
...
- fixed "Reset all GUI options" not resetting all GUI sections as expected [#2967]
- fixed sbie:// links below the Box Type presets [#2959]
- fixed "Reset all GUI options" causing all sandbox names to disappear [#2972]
...
- fixed Sandybox not marking deleted files or registry keys while virtualization scheme v2 is active (1.9.4/1.9.5) [#2984]
...
- added a limit to the number of identical log messages [#2856]

### Fixed
- fixed sbie:// links not working properly [#2959]
...
- fixed the checkbox for Open System Protected Storage appearing unchecked [#2866]
- fixed Firewall Rules' lack of colour contrast making text hard to read [#2900]
- fixed RecoverFolder shows GUID instead of folder name [#2918]
- fixed hourglass icon overlay gets stuck in the system tray when operation is stopped [#2869]
...
- fixed Running the Insert chart in Microsoft Word in the sandbox fails [#2863]
- fixed An error occurred in the switch sandbox type preset configuration [#2941]
- fixed Unable to open Microsoft Excel (thanks lmou523) [#2890]
...
- fixed Qt6 compatibility issue with 7-Zip file system implementation [#2681]
...
- fixed long-standing ping issue with compartment type boxes [#1608]
...
- avoid blocking Windows Explorer when dragging [#2660]
...
### Added
- added option to Sandybox Classic to apply a supporter certificate
...
### Fixed
- fixed issue with Microsoft Edge in a Security Hardened box [#2571]
...
- fixed minor issue during Sandybox uninstall [#2421]
...
- Sandybox no longer leaves behind permanent directory objects
...
- added support for Windows on ARM64 [#1321] [#645]
  ...
  - Note: when Sandybox is running, it disables the use of CHPE binaries for x86 processes globally - as required for the forced process functionality. This can be disabled by adding the global option "DisableCHPE=n" to the Sandboxie.ini, which will terminate x86 processes started outside the sandbox with message SBIE2338, instead of being forced
...
- added domain\user notation when the LogFile registry setting is applied as workaround for [#2207]
  - Usage: in "HKLM\SYSTEM\CurrentControlSet\Services\SbieSvc" add REG_SZ "LogFile" with "3;[path]\Sandybox.log"
...
### Changed
- Sandybox no longer issues message 1301 when forced processes are temporarily disabled
...
### Fixed
- fixed issue with unnecessary Sandybox config reloads introduced in 1.1.1 [#1938]
...
- double-click on the path column now opens the box root in Windows Explorer [#1924]
...
### Changed
- reworked the mechanism Sandybox uses to mark host files as deleted
...
- fixed editing of command line entry not being recognized in the Run Menu [#1648]
...
### Changed
- "HideHostProcess=program.exe" can now be used to hide Sandybox services [#1336]
...
- added version number to the title bar of Sandybox Classic
- added option to return not to a snapshot but to an empty box state while keeping all snapshots
- Sandboxie-Plus.ini can now be placed in C:\ProgramData\Sandybox\ folder and takes precedence (for business use)
...
- fixed issue with handling directory junctions in Sandybox [#1396]
...
- fixed taskbar not showing with persistent box border in full screen [post474](https://forum.xanasoft.com/threads/taskbar-problems-when-using-Sandybox-in-windows-11.224/post-474)
...
### Added
- replaced "Open with" with a Sandybox dialog to work on Windows 10 [#1138]
...
- added Plus installer option to start the default browser under Sandybox through a desktop shortcut
...
- reworked the resource access path matching mechanism to optionally apply more specific rules over less specific ones
  ...
  - added "NormalFilePath=..." to restore default Sandybox behaviour on a given path
...
### Fixed
- fixed SBIE1401 notification during Sandybox uninstall (by mpheath) [68fa37d]
...
- Plus installer will autostart SandMan.exe after install to fix a taskbar icon issue [#post-3040211](https://www.wilderssecurity.com/threads/Sandybox-0-9-7-test-build.440906/page-4#post-3040211)
...
### Fixed
- fixed inability to delete read-only files from the sandboxed Windows Explorer [#1237]
...
- added "SbieTrace=y" option to trace the interaction between Sandybox processes and Sandybox core components
...
- fixed KmdUtil warning 1061 after Plus upgrade (by mpheath) [#968] [#1139]
...
### Added
- ability to use the "Run Unsandboxed" option with Sandybox links [#614]

### Fixed
- fixed "Run Outside Sandbox" issue on Classic builds [#614]
...
- fixed Inno Setup script not being able to remove shell integration keys during Sandybox uninstall (by mpheath) [#1037]
...
- rolled back the switch from using NtQueryKey to NtQueryObject as it seems to break Windows 10 version 1803 and older [#984]
...
### Added
- normally Sandybox applies "Close...=!<program>,..." directives to non-excluded images if they are located in a sandbox
...
- fixed issue when using runas to start a process that was created outside of the Sandybox supervision [#688]
...
- fixed Sandybox Classic crash when saving any option in Sandbox Settings -> Appearance (by typpos) [#586]
...
- the LogAPI library is now using Sandybox's tracing facility to log events instead of its own pipe server
...
- added option to keep the SandMan UI always on top [#352]
...
### Added
- FIXED SECURITY ISSUE ID-4: Sandybox now strips particularly problematic privileges from sandboxed system tokens
...
- fixed MSI installer not being able to create "C:\Config.msi" folder on Windows 20H2 [#219]
...
### Changed
- changed docs and update URLs to the new Sandybox.com domain
- greatly improved the setup script (thanks mpheath)
- "OpenClsid=" and "ClosedClsid=" now support specifying a program or group name
- by default, when started in portable mode, the sandbox folder will be located in the parent directory of the Sandybox instance
...
- fixed sandboxed Windows Explorer issue [#289]
...
- added ability to prevent system wide process starts, Sandybox can now instead of just alerting also block processed on the alert list
...
### Changed
- AppUserModelID string no longer contains Sandybox version string
...
- added Sandbox Presets submenu to quickly change some settings
  - Enable/Disable API logging; LogAPI DLLs are now distributed with Sandybox
  - Drop admin rights
...
- added "SandboxieLogon=y" - it makes processes run under the SID of the "Sandybox" user instead of the Anonymous user
  - Note: the global option "AllowSandboxieLogon=y" must be enabled, the "Sandybox" user account must be manually created first and the driver reloaded, else process start will fail
...
- added Maintenance menu to the Sandbox menu, allowing to install/uninstall and start/stop Sandybox driver, service
...
### Added
- created a new Qt-based UI named SandMan (Sandybox Manager)
