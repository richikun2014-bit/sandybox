...
### 新增
- 新增选项 'BlockLocalLoop=y' ，用于阻止本地回环连接
- 新增 Electron 检测功能，并在遇到问题时，显示 SBIE2189 消息及故障排查选项，可为遇到的应用程序设置 'SpecialImage=chrome,program.exe'
- 新增自动化的 Electron 应用检测启发式算法，默认启用，可通过设置 'UseElectronDetection=n' 禁用

### 变更
- 硬件信息防护现在可以针对每个进程进行切换 [#5271]
- 驱动现在使用 UTC 时间而非本地时间进行证书验证 [#4241] [#5316]

### 修复
- 修复易失性配置更新时的问题
- 修复并补充缺失的 WaitServiceState，解决了“LINE 无法启动”/ 无签名 的问题 [#5306]
- 修复（ipc）：在 NtQueryDirectoryObject 中 Ipc_GetName 失败时，释放 Ipc_Handles_CritSec [#5326]
- 修复所有基于 Tauri 构建的 WebView2 应用程序用户界面无响应的问题 [#5327]
- 修复驱动卸载过程中竞态条件导致蓝屏死机 (BSOD) 的问题 [#5329]
- 修复 Key_MergeSubkeys 逻辑校验问题 [#5301]
- 修复应用程序隔离模式下 File_WaitNamedPipe 的相关问题
...
### 新增
- 新增可配置的窗口位置设置，用于控制主窗口、非主窗口、文件恢复窗口以及通知窗口打开时所在的显示器，并支持可选的回退模式。[#4536] [#5238]
  - 全局设置 > 界面配置 > 窗口选项
- 新增“仅显示标签”边框模式选项（`onlbl`、`ttllbl`、`alllbl`），该模式隐藏彩色边框，仅显示沙箱名称（或别名）标签 [#5239]
- 针对 Windows 11 新增 MIDI 兼容性模板 [#5183] [#5203]（感谢 xsmolasses）
- 新增系统托盘自定义选项（全局设置 > Shell 集成 > 系统托盘）：[#5254]
  - “在托盘上下文菜单中显示图标”（`Options/TrayIcons`）—— 控制是否在托盘菜单中显示自定义沙箱图标
  - “在托盘中显示沙箱别名而非名称”（`Options/TrayUseAlias`）—— 在精简型和常规托盘菜单中显示已配置的别名/显示名称
  - “在托盘列表中以工具提示显示沙箱状态”（`Options/TrayStatusTip`）现支持三态设置：未勾选 = 从不，部分 = 按住 Ctrl 或 Shift 时（默认），已勾选 = 始终
  - “在托盘列表中为沙箱显示叠加图标”（`Options/TrayOverlayIcons`）—— 在托盘图标上显示与主沙箱列表相同的沙箱状态叠加（未强制、磁盘镜像挂载/卸载、RAM 磁盘、自动删除），适用于精简组件及常规菜单
- 新增通过 `HideBordersFromCapture` 进行边框捕捉排除
  - 可在屏幕截图和屏幕录制中避免捕捉到边框及标签，默认对 `CoverBoxedWindows` 生效
- 新增边框标签宽度和任务栏裁剪选项
  - 通过 `BorderColor` 标签宽度和 `BorderExcludeTaskbar` 进行配置
- 新增 `UseFakeShellDispatch` 选项，提供合成的 `IShellDispatch` 回退策略（可修复部分 WebView2 问题）
  - 可通过 `UseFakeShellDispatch=process,n` 按进程禁用
- 新增 `Template_OnScreenKeyboard`（Windows 11），修复与沙箱程序一起使用时屏幕键盘冻结的问题

### 变更
- 通过在沙箱模型中缓存自定义 `BoxIcon` 解析结果，而非每次刷新时重新加载图标资源，减少了 GUI 持续 CPU 占用
- 沙盘管理器主定时器中的互联网连通性检测降低为每 60 秒一次，并缓存检测结果
  - 只有设备具备网络连接时才运行更新程序，避免无效的网络请求并减少反复检查 `HKLM\\SYSTEM\\Setup\\SystemSetupInProgress` 注册表
- 修改重复沙箱行为，现活动沙箱别名在复制时也会添加“Copy”后缀
- 托盘中沙箱/分组排序方式同步主沙箱列表模式（手动/升序/降序），包含分组排序
- 托盘和沙箱子菜单的图标缓存优化，仅在缓存未命中时解析 `DblClickAction` 目标路径（`GetCommandFile`），并缓存 `LoadWindowsIcon(path,index)` 结果，减少打开菜单时重复提取系统图标
- 优化了边框渲染效果

### 修复
- 修复了在网络选项页面下，将未列出的进程的网络模式设为非默认后离开页面时，错误弹出的“尚有变更未保存”提示
- 修复了复制后的沙箱未保留原有的沙箱分组归属问题
- 修复了在分组下双击空路径/命令行导致崩溃的问题 [#5253]
- 修复了精简托盘沙箱列表对于沙箱名称过长的裁切问题，现根据字体度量为每项精确计算宽度，并可在任意 DPI 正确缩放 [#5254]
- 修复了 `SbieDll` 中 WOW64 注册表视图在开启相对键时的继承问题，父级 `KEY_WOW64_32KEY/KEY_WOW64_64KEY` 语义跨 `NtOpenKey`/`NtCreateKey` 保持一致 [#5171] [#5244]
...
- 修复 `ClosedClsid` 与 `ClosedRT` 设置的解析逻辑 [#5263]
...
- 修复 Sandybox 通过 INI CRLF 注入绕过 EditAdminOnly 的问题（由 sammy12342 报告）
...
- 重构了 SandMan 中的 `重命名沙箱` 对话框：添加了专用的别名控制项，持久化保存了 `隐藏别名` 偏好设置，改进了稳定/动态对话框大小调整，现在禁用的别名将以 `BoxAliasDisabled` 的形式保存 [#4657] [#5231]

### 修复
- 修复了当边框标签文本被禁用（`no` 模式）时，边框覆盖无法消除的问题 [#5230]
- 修复了当前聚焦窗口变为无效/不可见时，边框仍然显示的问题
- 修复了 `all` 边框模式下透明度更新无法一致应用所配置 alpha 的问题
- 修复了在模板设置的 Config Dump 标签下出现重复的 Name 列的问题 [#4900] [#5232]
...
### 修复
- 修复了在 Windows 10 上由 1.17.0 版本引入的 SbieSvc IPC 回归问题 [#5226]
...
- 改进了 RenameSection 的处理，以保留注释和原始节顺序 [#5220]

### 修复
- 修复了启动 UWP 应用沙箱时注册表挂载失败的问题
- 修复了 API CryptUnprotectData 始终返回错误的 ppszDataDescr [#5191]
- 修复了 SbieDll.dll 中的句柄泄漏问题
- 修复了视图中切换沙箱排序的相关问题 [#5201]（感谢 Catreap）
- 修复了 Microsoft Edge 144 在应用隔离沙箱中崩溃的问题 [#5188]
- 修复了在沙箱中进行多次文件搜索时文件搜索栏崩溃的问题 [#5166] [#5221]
...
- 修复 SbieDll.dll 中与 SbieApi_DeviceHandle 相关的潜在句柄泄漏问题。 [#5097]
...
### 修复
- 修复在 Windows 7 上无法连接驱动的问题 [#5068]
- 修复 'Actual Budget' 不再工作的问题 [#5071]
...
- 保留自定义沙盒颜色：为沙盒设置的自定义颜色在更改 `Box Type Preset` 后仍将保留 [#5025]（感谢 Pinefone）
- 为沙盒边框设置新增 Alpha 透明度 [#5037]
- 用户界面：启动时默认折叠所有分组 [#5052]
- 用户界面：新增菜单选项，可折叠/展开分组 [#5053]
...
### 修复
- 修复当 VPN/绑定适配器断开连接时 BindAdapter 的可靠性问题 [#5055] [#4896]
- 修复 WOW6432Node 错误，在必要创建第二个不存在的子键时变为 `HKLM\SOFTWARE\WOW6432Node\WOW6432Node` [#5057]
- 修复 PowerShell 能够在添加到程序控制的文件夹中启动可执行文件的问题 [#5027]
...
### 修复
- 修复 **Sandybox 1.16.3** 在 Windows 预览版 Build 27943 无法连接驱动的问题 (#5003)
- 修复最新版本 Signal 崩溃的问题
...
- 修复安装程序未清理旧版 Sandybox 1.16.x 遗留文件的问题 (#4934)
- 修复以管理员身份重启时未显示窗口的问题
...
- 为强制进程新增了通配符支持 [#2603]
- 在跟踪日志页面的 “类型” 下拉菜单中，新增了文本缩放的正确支持，防止在应用显示缩放时文本被截断 [#4812]（感谢 wzxjohn）
- 新增了对 EventPair 和 KeyedEvent 对象的虚拟化处理
- 为 Timer 对象新增了筛选和虚拟化功能
- 新增了创建虚拟沙盘（不会存储在 Sandybox ini 文件中）的选项
- 新增了 `UseSandboxieUAC=y` 选项，可以使用沙盘内置的 UAC 替代系统默认的 UAC 提示
  - 注意：新的 UAC 提示还提供仅授予应用程序“伪管理员权限”的选项

### 更改
- 重构了 **DNS 过滤器逻辑**，即便未发生 DNS 解析，或者未返回有效 IP 地址时也能进行过滤 [#4811]（感谢 wzxjohn）
  - 之前只有在发起了 DNS 请求并收到有效响应（如 IP 地址）时，DNS 过滤才会生效
  - 这会导致诸如未解析域名无法被修改和在过滤期间潜在的信息泄露等问题
- 所有组件均启用了 `/GS`
- 驱动程序启用了 `/CETCOMPAT`
- 重构了 ini 配置文件的处理逻辑 [#4492]
  - 新增 `ImportBox=C:\path\To\Box.ini`
  - 新增了无需 ini 文件即可在驱动层创建虚拟配置区段的机制
- 改进了移动沙箱右键菜单 [#4819]

### 修复
- 在 Firefox 138 及以上版本全屏时，任务栏依然显示在顶部的问题 [#4761]
- 修复了删除多项内容时的多余选项 [#4817]
- 修复了 Sandybox 意外关闭的问题
- 修复了页面显示异常 [#4810]
- 修复了在 Windows 11 上通过右键菜单运行程序时的工作目录问题 [#4844]（感谢 micbou）


## [1.15.12 / 5.70.12] - 2025-05-01

### 新增
- 为 Plus 界面新增印度尼西亚语翻译 [4bad4d1]（感谢 ？？？）
- 新增 WeType 输入法（IME）模板 [#4715]（感谢 bot-1450）
- 在代理配置中新增主机名支持（在 WSA_Startup 时查找主机）
- Sandybox 现在支持绑定到适配器，而非仅绑定 IP（在 WSA_Startup 时解析 IP）
- 新增对 %ProgramFiles(x86)% 变量的支持 [#2402]
- 新增 TemplateDefaultFolders，包含所有需在增强隐私盒中创建的默认路径

### 更改
- 修正了沙盒列表中的异常排序问题 [#4615]
- 支持标签页中的名称与 HwID 信息默认隐藏，可按需切换显示 [#4714]
- 新增可选的代理中继实现（不再篡改现有套接字，而是启动中继线程）[实验性]

### 修复
- 修复了“在沙盒中运行”选择提示未显示沙盒别名的问题 [#4709]
- 修复了使用 Ctrl+F 后，沙盒分组的展开和关闭记录被打乱的问题 [#4708]
- 为注册表的 set/get 函数添加了额外的检查
- 修复了 Pool_Alloc 中的问题
- 修复了 SbieCtrl.exe 在 Process Explorer 中持续维持 3.2MB/s IO 速率的问题 [#4693]
- 修复了腾讯 TIM 模板导致无法使用拖放功能的问题 [#4688]
- 改进了 Sandboxie.ini 文件很大时响应速度变慢的问题 [#4573]
- 修复了 Msi 安装过程中无法在数据保护沙盒中创建 AppData\Romaing\Microsoft 文件夹的问题 [#4711]
- 修复了 Sandybox 在时间加速相关的两个错误，并新增了两个时间函数的钩子 [#4721]（感谢 pwnmelife）
- 修复了协议选择的翻译字符串被保存到配置文件中的问题
- 修复了沙盒选择器图标
- 修复了切换 UI 语言时工具栏未更新的问题 [#4726]
- 修复了 PingInfoView 在 Sandybox 中运行时报错的问题 [#4718]
- 修复了卸载注册表项未找到时，CollectProducts() 卡在循环中的问题 [#4753]
- 修复了与 SbieDll.dll 相关的崩溃问题 [#4754]
- 修复了在创建快捷方式时自动下载文件的问题 [#4663] [4750]（感谢 WZ-Tong）

### 移除
- 移除了无效的“删除内容”按钮 [#4720]（感谢 habatake）
...
### 修复
- 修复了 Firefox 137.0 在 Sandybox v1.15.9 中生成 SBIE2328 通知的问题 [#4652][#4652][#4640]
- 修复了 1.15.9 中缺少 SbieShellExt.dll 的问题
- 修复了 Process_GetCommandLine 导致内存泄漏的问题 [#4658]
- 修复了 Thunderbird 102.15.1 在 Sandybox v1.15.9 下未加载所有扩展的问题 [#4653]
- 修复了在 Sandybox 中，Invoke SetParent 将窗口更改为仅消息窗口失败的问题 [#4656]
- 修复了检索评估证书时处理 Unicode 名称的问题
- 修复了删除内容消息框是否应始终在最上方的问题 [#4673]


## [1.15.9 / 5.70.9] - 2025-03-??

### 新增
- 复制沙箱，并从 UI 中获取内容 [#4542]
- 添加新的不安全调试选项 'OpenAllSysCalls=y'
- 添加 WindowGrid 模板
- 添加新的证书类型 [DEVELOPER](https://xanasoft.com/product/Sandybox-developer/)，它可以禁用用户模式组件验证，但只能以节点锁定的形式使用。
...
### 修复
- 修复了框删除/移除确认对话框中与 bullet 字符相关的编码问题 [#4591]
- 更改箱的设置/类型会导致一些模板设置在箱中重复 [#4595]
- 在尝试将沙箱放置在分区根目录时添加警告 [#4582]
- 修复了卸载时的错误 [#4540]
- 修复了在 Sandybox 中运行时 Golang Cmd.Run() 不返回的问题 [#4538]
  - 修复了可能帮助许多其他应用程序的通用句柄重复问题
- 改进了当 Sandboxie.ini 文件过大时，响应速度变差的问题 [#4573]
- 修复了在特定安装中的 Sandybox 冻结/崩溃问题 [#4537]
  - 当 sandman 挂起并被终止时，下次启动时会询问是否禁用软件兼容性检查的更新扫描
- 修复了与钩子监视器显示有关的问题
- 修复了 ini 节区编辑的问题，eqc 不再关闭窗口
- 修复了在 Firefox 137 Nightly 及后续版本中引入的钩子问题 [#4522]
- 修复了不安全调试选项 'OriginalToken=y' 的问题
- 框组名称意外输入了换行符 [#4608]
- 修复了在新箱向导中从路径选择下拉菜单选择的路径返回“不是有效路径”错误的问题 [#4611]
...
### 修复
- 修复了 "WriteFilePath" 似乎工作不正确的问题 [#4553]
- 修复了沙盒备注，在保存时会删除空行的问题 [#4547]
...
### 更改
- 更新 Notepad++ 模板以在 Sandybox Classic 中工作
- 更新 MiscHelpers.dll，改进查找器

### 修复
- 修复 Sandybox 安装程序中的过时条目
- 修复从“开始菜单运行”创建的固定快捷方式无法使用的问题 [#4502]
- 修复在资源低的情况下驱动程序潜在的蓝屏死机问题
- 修复 NsiAllocateAndGetTable 碰到崩溃的问题（感谢 tzf-omkey)[#4511]
- 修复当沙箱进程接收到设备更改通知时的内存泄漏
- 修复在隐私增强的沙箱中 NtQueryDirectoryFile 挂钩导致的内存泄漏问题 [#4509]
...
### 修复
- 修复 Sandybox 数据文件夹在更新至版本 1.15.5 后未能正常工作的问题
- 修复故障排除引擎中持锁过长的问题
- 修复 Adobe Acrobat Reader 在应用程序隔离箱中崩溃的问题 [#4363]
- 修复 Viber 软件已更新，但在使用 Sandybox 程序打开时，系统程序始终崩溃的问题 [#4298]
- 修复无法在 Sandybox 中运行 qutebrowser 的问题 [#4419]
- 修复在版本 1.15.5 中启用 ApiTrace 时无法在沙箱中启动应用程序的问题 [#4488]
...
### 更改
- 将 Sandybox 数据文件夹从 %LOCALAPPDATA%\\Sandybox 移动到 %LOCALAPPDATA%\\Xanasoft\\Sandybox
  - 注意：文件夹将自动移动。当降级至旧版本时，需要手动移动回去
- 重新排序沙箱上下文菜单选项 [#4444]

### 修复
- 修复以非高级视图按下以管理员身份运行时的崩溃问题
- 修复最新 Windows 11 ARM64 上所有应用程序的 WerFault.exe 错误 [#4422]
- 修复 ARM64 上阻止 x64 可执行文件运行的问题 [#4415]
- 修复进程自我终止的问题
- 修复 Windows 10 ARM64 上找不到 msedge_elf.dll 的问题
- 修复 Windows 10 ARM64 的兼容性问题 [#3600]
- 修复当 Gui_ConnectConsole 等待 SbieSvc.exe 退出时的无限循环问题 [#4462]（感谢 bot-1450)
- 修复在 RunCommand 设置中的命令路径包含空格时程序图标不显示的问题 [#4416]
- 修复模板扫描无法正常工作的问题 [#4401]
- 修复通过 mstsc.exe 连接到 RDP 服务器时加密沙箱中凭据对话框不显示的问题 [#4389]
- 修复文件夹恢复仅移动该文件夹内部文件的问题 [#4453]
- 修复查询文件属性失败导致程序异常的问题 [#4457](感谢 lmou523)
- 修复 DNS 过滤中未定义域名的 IP 导致应用程序崩溃的问题 [#4475]
- 修复从“运行程序”菜单启动时文件未写入的问题 [#4398]


## [1.15.4 / 5.70.4] - 2024-12-19

### 新增
- 为 Joplin 添加模板 [#4402]（感谢 offhub)

### 修复
- 修复 INI 编辑器的暗模式
- 修复删除沙箱内容的问题 [#4407]（感谢 bot-1450)
- 修复从沙箱选择窗口中“无沙箱运行”不再工作的的问题 [#4403]
- 修复在启用 `ProtectHostImages=y` 的情况下，Firefox 标签页崩溃的问题 [#4394]
  - 仅考虑默认安装位置；在这些位置之外安装的基于 Firefox 的浏览器可能仍会崩溃
  - 为防止此问题，请手动为自定义安装路径在配置中添加 `DontCopy=<CustomInstallPath>`
- 修复当从 AlertFolder 打开 DLL 时使用 x64dbg 出现的蓝屏死机 "SYSTEM_SERVICE_EXCEPTION (3b)" 问题 [#4421]
- 修复当终止所有沙箱程序时出现的蓝屏死机 "CRITICAL_PROCESS_DIED" 问题 [#1316]
  - 注意：我们现在逐个终止被沙箱的进程，而不是使用作业对象，除非设置了 "TerminateJobObject=y"
- 修复 INI 编辑器在添加高亮功能后字体选择无法工作的的问题 [#4429]
...
- 添加 SBIE1321 以记录所有强制进程事件，可以通过 "NotifyForceProcessEnabled=y" 启用，改进 [#4113]
- 添加 Sandybox UI 的自定义字体配置 [#4397]（感谢 habatake)

### 更改
- 改进支持通知
- 改进卸载 Sandybox 残余文件的问题 [#4374]
...
- 添加同时在多个沙箱中打开程序的功能 [#4231]
- 在沙箱设置中添加 "描述（Description）" 字段 [#4243]
- 添加 "NotifyMsiInstaller=y"，默认启用以在沙箱中运行没有推荐排除项的 MSI 安装程序时显示消息 SBIE2194 [#4330]
  - SBIE2194： MSI 安装程序需要在 INI 中设置 'MsiInstallerExemptions=y' 选项才能正确工作，然而此选项会削弱隔离。
- 添加隐藏已安装程序的选项 [#4139]
- 添加隐藏托盘图标的选项 [#4075]
- 添加改进的跟踪日志过滤功能 [#4338]
- 添加 SbieMessages 的事件日志监控 [#4113]
  - 将 'LogMessageEvents=y' 添加到全局设置，以将所有 Sbie 事件记录到系统事件日志中

### 更改
- 验证与 Windows build 27749 的兼容性并更新 DynData
- 通过拖放运行时，现将应用程序的父文件夹用作工作目录 [#4073]
- 更新至 Qt 5.15.16 和 OpenSSL 3.4.0 [#4370]（感谢 offhub)

### 修复
- 修复 Sandybox 安装或更新期间 .tmp 文件签名的问题 [#2643] [#4343]
- 修复 DLL 卸载问题
- 修复 "Files Resource Access" 授予对排除文件夹的访问问题 [#4007]
- 修复当设置 "ForceDisableAdminOnly=y" 时，“无沙箱运行”从运行菜单中消失的问题 [#4233]
- 修复重命名文件或文件夹时在空操作条件下的死锁问题 [#4304]
- 修复移动沙箱文件或文件夹的问题 [#4329]
- 修复 Sandybox 导致 Firefox Nightly 崩溃的问题 [#4183]
- “从快速预览中沙箱运行”应只有一个选项 [#4339]

## [1.15.1 / 5.70.1] - 2024-10-29

### 更改
- 验证了与 Windows 27729 构建的兼容性并更新了 DynData
- 更新了 Templates.ini，以授权访问多媒体类调度服务 [#4312]（感谢 offhub)
- 更新至 Inno Setup 6.3.3 [#4020]

### 修复
- 修复了 Sandybox 加密在红框中无法启动的问题
...
- 新增选项 'OpenWPADEndpoint=y' [#4292]

### 更改
- 改善了 SandboxieCrypto 启动性能
- 改善了沙箱中 RPCSS 的启动性能
- 更新至 Qt 5.15.15 和 OpenSSL 3.3.2 [#4223]（感谢 offhub)
- 设置了 UI 控件的标签顺序和伙伴 [#4300]（感谢 gexgd0419)

### 修复
- 修复了 ImDiskApp 卸载键总是写入注册表的问题 [#4282]
- 通过添加新的 ACL 处理修复了安全问题 ID-24 [CVE-2024-49360]
  - 注意：在多用户系统上设置 'LockBoxToUser=y'

## [1.14.10 / 5.69.10] - 2024-10-03

### 新增
- 新增直接导入加密归档文件的功能 [#4255]
...
### 修复
- 修复了沙箱路径条目组合框的问题
- 修复了 GetRawInputDeviceInfoW() 代理引起的缓冲区溢出问题 [#4267]（感谢 marti4d)
...
### 修复
- 修复了导出的加密归档文件无法被 Sandybox 解压的问题 [#4229]

## [1.14.8 / 5.69.8] - 2024-09-09

### 更改
- 允许用户使用 .zip 文件导入/导出箱 [#4200]
...
- 新增 "TerminateWhenExit" (bool，在 Sandboxie-Plus.ini 中) 在 SandMan 退出时终止所有进程 [#4171]
- 新增询问箱导入位置的问答框 [#4169]
- 新增用于配置 DropConHostIntegrity 的 UI 选项
- 新增 "HideNetworkAdapterMAC" (bool)，在应用程序尝试获取网络适配器 MAC 地址时返回随机值
- 新增共享模板选择到新箱向导的高级选项中的共享模板功能 [#4199]
  - 可用的共享模板数量增加到 10 个
  - 若要更新列表中显示的名称，只需调整每个模板中的 "Tmpl.Title" 设置

### 修复
- 修复和改善 HideDiskSerialNumber 选项导致应用程序崩溃的问题 [#4185]
- 修复了加密代理密码格式不正确的问题 [#4197]
- 修复了 NtQueryDirectoryObject（不应返回 "STATUS_MORE_ENTRIES")，因为这是一种简单的沙箱检测 [#4201]
...
### 修复
- 修复 Windows 7 下的 CustomLCID 问题 [#4117]
...
### 新增
- 新增 HwID 显示
- 新增语言欺骗 "CustomLCID=1033" [#4024]（感谢 Yeyixiao)
- 新增选项以始终以管理员身份运行 SandMan UI [#4090]
- 新增代理排除功能 [#4036]
- 新增 "ForceChildren=Program.exe" [#4070]
...
- 修复了 "HideFirmwareInfo=y" 实现的错误
  ...
  - 新增 UI 选项
- 修复了加密沙箱中的 schannel 错误 SEC_E_SECPKG_NOT_FOUND [#4081]
- 修复了 "沙箱名称过长" 的错误，导致 Sbie2327 中出现异常 [#4064]
- 修复了作业对象的内存限制，现在可以超过 4GB [#4096]
...
### 修复
- 修复 "开始限制" 选项卡布局破损的问题 [#4045]
- 修复了管理员无法更改沙箱配置的问题 [#4057] [#4068]

## [1.14.3 / 5.69.3] - 2024-07-01

### 更改
- 更新至 Qt 5.15.14 和 OpenSSL 3.3.1 [#3994]（感谢 offhub)

### 修复
- 修复了使用 MSFT 账户时，沙箱中无法以管理员身份启动应用程序的问题 [#4022]
- 修复了 Sbie 1.14.1 和 1.14.2 中的 Firefox 问题 [#4012]
  - 回滚了在 1.14.1 中添加的驱动程序验证器修复
- 修复了 CustomChromiumFlags 和 --single-argument 的问题 [#4033]
- 修复了在启用 UseCreateToken/SandboxieAllGroup 时，Sandybox 程序在关闭以管理员身份运行的程序后未终止的问题 [#4030]

## [1.14.2 / 5.69.2] - 2024-06-19

### 新增
- 新增 SbieIni 选项以修改受密码保护的配置 [#3903]
...
- 新增选项 "HideSbieProcesses"，以隐藏 Sandybox 工作进程（SbieSvc、SandboxieRpcSs 等)
...
- 新增新选项 "AllowCoverTaskbar" [#3975]
- 新增 RPC 端口消息过滤机制，以阻止驱动程序通过 RDP 进行不安全的调用 [#3930]
...
- 修复了导出沙箱功能未包含隐藏文件的问题 [#3980]（感谢 L4cache)
- 修复了 Chrome 打印机停止工作的问题 [#3926]
  - Sandybox 将在基于 Chromium 的浏览器命令行中添加 CustomChromiumFlags=--disable-features=PrintCompositorLPAC
  - 注意：当前 Less Privileged App Container (LPAC) 与 Sandybox 不兼容
- 修复了访问相对符号链接时问题，目标以点开头 [#3981]
- 修复了通过双击系统托盘上下文窗口打开沙箱属性窗口的问题 [#3861]
- 修复了自版本 1.12.9 后强制程序的启动延迟问题 [#3868]
  - 此问题是在 1.13.0 中引入的，可能影响了其他用例导致各种问题
- 修复了杂项选项列表的问题
- 改善了与沙箱运行的 Steam 的兼容性
- 修复了与 FakeAdminRights 的兼容性问题 [#3989]（感谢 offhub)

## [1.14.1 / 5.69.1] - 2024-06-06

### 新增
- 新增将 "Sandboxie\All Sandboxes" SID 添加至使用 SandboxieLogon 的令牌中 [#3191]
...
- 新增选项 "AlertBeforeStart"
  - 当设置时，在使用 "Start.exe" 启动新程序进入沙箱时，会弹出提示，并检查启动 "Start.exe" 的程序是否为 Sandybox 组件，如果不是，则弹出警告
...
- 修复了代理身份验证设置的问题
- 修复了 SbieSvc 中的内存泄漏
- 修复了 WFP 选项应用不一致的问题 [#3900]
- 修复了缓冲区哈希函数中的资源泄漏
- 修复了启用 BlockInterferenceControl 时 DLL 名称损坏的问题 [#3945]
- 修复了驱动程序验证器的问题
...
### 新增
- 在 Sandybox 关于对话框中增加 SbieDll.dll 和 SbieSvc.exe 的文件版本信息

### 修改
- 改进 SandMan 中关于 DropAdminRights 的复选框 [#3851] （感谢 offhub)

### 修复
- 修复了文件的符号链接 [#3852]
- 修复了启动代理选项的问题 [#3844]（感谢 offhub)
...
- 增加可编辑的触发器列表 [#3742]
- 增加了可选的屏幕截图保护扩展到用户界面 [#3739]
- 增加了一个按钮来编辑本地/自定义模板 [#3738]
- 增加 "Run Sandboxed" 窗口的可调整大小 [#3697]
- 增加 Notepad++ 模板 [#3836]（感谢 offhub)

### 修改
- 改进了 Avast 模板 [#3824]（感谢 offhub)
- 重命名了一些实验性选项，并在用户界面中将其标记为实验性
  - "IsBlockCapture=y" -> "BlockScreenCapture=y"
  - "IsProtectScreen=>" -> "CoverBoxedWindows=y"

### 修复
- 修复了更改 BlockDNS 或 BlockPorts 选项时无效的应用按钮 [#3807]
- 修复了新 Qt 导致的故障排除向导中断问题 [#3810]
- 修复了设置对话框未显示正确的 RAM 驱动器字母
- 修复由于最新 Qt 构建缺少 SSL 支持而导致的更新程序故障 [#3810]
- 修复启用 "DropAdminRights/FakeAdminRights" 后将 "BlockInterferePower 和 ForceProtectionOnMount" 添加到 INI 的问题 [#3825]
- 修复 KeePass 由于 "BlockScreenCapture=y" 导致的 "Out of Memory" 崩溃问题 [#3768]
- 修复在 Windows 7 中使用 IsBlockCapture=y 的 Sandybox 1.13.4 无法正常工作的问题 [#3769]
- 修复 explorer.exe 中的 "FakeAdminRights=y" 问题 [#3638]
- 修复强制文件夹警告通知 [#3569]
...
- 增加 Weasel 模板 [#3806]（感谢 xWTF)

### 修改
- 在测试签名模式下，Sandybox 将默认尝试过时的偏移量
- 更新为 Qt 5.15.13，并应用最新的安全补丁 [#3694]（感谢 LumitoLuma)
- 将网络限制从一般限制移动到网络页面上的单独标签
- 改进证书检索用户界面消息
- 改进 MPC-BE 模板 [#3798]（感谢 offhub)

### 修复
- 修复了虚拟化方案版本 2 导致极慢的文件删除速度 [#3650]

### 删除
- 删除了 Templates.ini 中过时的推荐 [#3802]
...
### 新增
- 增加防止沙箱化进程访问沙箱外窗口图像的选项 [#1985]（感谢 Yeyixiao)
...
- 增加选项 'SharedTemplate' 到 Box Wizard [#3737]（感谢 offhub)
- 增加强制启用加密沙箱保护的选项 [#3736]（感谢 Yeyixiao)
- 增加一个菜单和按钮/图标来挂起所有进程 [#3741] ()

### 修改
- 选项 "LingerLeniency=n" 现在也禁用新启动的滞留进程的 5 秒宽限期 [#1892]
...
### 新增
- 增加菜单项以管理员身份重新启动 SandMan [#3581]（感谢 Yeyixiao)
...
- 增加防止沙箱化进程干预电源操作的选项 [#3640]（感谢 Yeyixiao)
  - 可以通过 "BlockInterferePower=y" 启用
  - 请参见 SandMan 用户界面中的沙箱选项 "Prevent sandboxed processes from interfering with power operations"
- 增加新的模式机制，使用 `**` 作为占位符表示不包含 `\` 的任意路径元素 [1ff2867]

### 修改
- 重新设计了 SandMan 中挂起所有进程的选项（在 1.13.1 中引入) [#3582]

### 修复
- 修复隐私模式、NormalFilePath 和符号链接问题 [#3660]
- 修复在应用空间沙箱中访问回收站的问题 [#3665]
...
### 新增
- 增加在 SandMan 中挂起所有进程的选项 [#3582]（感谢 Yeyixiao)
- 增加 "On Terminate" 触发器 [#3584]（感谢 Yeyixiao)

### 修改
- 更改了 DynData 格式以添加标志
- 恢复了没有驱动器字母的卷的新沙箱目录结构 [#3632]
...
- 修复符号链接问题 [#3537]
- 修复在应用空间沙箱中文件重定向问题 [#3637]
- 修复与隔离模式兼容性回退的问题
- 修复缺失的最大密码长度检查 [#3639]
- 修复在没有驱动器字母的卷中从沙箱启动可执行文件的问题，兼容 Windows 1803 及更早版本 [#3627]
...
### 新增
- 在 Plus 用户界面中增加日语语言支持 [#3573]

### 修复
- 修复警告问题：`SBIE2321 Cannot manage device map: C0000034 / 11` [#2996]
- 修复在模拟管理员沙箱中无法运行 explorer.exe 的问题 [#3516]
- 修复启用追踪日志时与 WFP 相关的潜在 BSoD 问题 [#2471]
- 修复在运行菜单中固定程序未使用相同工作目录的问题 [#3555]
- 修复 UseNewSymlinkResolver 导致应用程序同时创建链接和目标文件夹的问题 [#3481]
- 修复 Plus 用户界面通知窗口在显示模态对话框时卡住的问题 [#3586]
...
### 修复
- 修复 "Duplicate Box Config" 选项无法保留原沙箱配置顺序的问题 [#3523]
- 修复在启用 "Configure Advanced Options" 时 "Save options as new defaults" 选项未出现的问题 [#3522]
- 修复在启用数据保护的 Windows 11 沙箱中运行 Windows 资源管理器的问题 [#3517]
  - 隐私箱的默认模板现在设置为 `NormalFilePath=%ProgramData%\Microsoft\*`
- 修复在安全加固沙箱中，使用 MB_DEFAULT_DESKTOP_ONLY 或 MB_SERVICE_NOTIFICATION 的消息框无法正确显示标题和文本的问题 [#3529]
- 修复在安全加固沙箱中 npm 无法运行的问题 [#3505]
- 修复在 1.12.6 中引入的符号链接和打开路径问题 [#3537]
  - 当遇到没有关联驱动器字母的卷时，Sandybox 使用 `\drive\{guid}` 代替 `\drive\[letter]`
  - 注意：如果随后该卷被分配了驱动器字母，在 `\drive\{guid}` 下的数据将被忽略！
- 修复在右键单击使用 Sandybox 创建的快捷方式时 "Run Unsandboxed" 命令的问题 [#3528]
  - 注意：为了使修复生效，需重新应用 Shell 集成
- 修复 `Error Status: 0x0000065b (Function failed during execution)` [#3504]
- 修复隐私增强沙箱因错误代码 SBIE2204 失败的问题 [#3542]
- 修复 OpenFilePath 目录未在其父文件夹中列举的问题 [#3519]
...
### 修复
- 修复 1.12.4 中引入的与 Start.exe 在加密箱中无法运行的问题 [#3514]
- 修复加密私有箱中启动进程时出现 "目录名无效" 的问题 [#3475]
- 修复在沙箱内部创建的符号链接无法正常工作的问题 [#3181]
- 修复对多个文件夹或一个驱动器字母与一个文件夹挂载的驱动器的问题
- 修复在使用 "SysCallLockDown=y" 的 Windows 11 上 explorer.exe 的问题 [#3516]
...
### 修复
- 修复了影响基于 Chromium 的浏览器的错误，涉及 "--type=crashpad-handler" 实例 [#post-3177139](https://www.wilderssecurity.com/threads/Sandybox-1-12-5.453131/#post-3177139)
...
### 新增
- 在 Sbie 消息选项卡中增加日期和时间的显示 [#3429]

### 修改
- 在没有活跃的支持者证书的情况下，稳定通道将不再提供自动更新
  - 自动更新程序仍会通知有关新稳定版本的信息，并指导用户手动下载最新的安装程序
- SandMan 用户界面中的清理按钮现在即使在未连接到 Sandybox 核心组件时也可用
- 箱创建向导现在允许基于任何其他箱类型创建加密沙箱

### 修复
- 修复在 ImDisk 卷中运行沙箱化进程的问题 [#3472]
- 修复样本 634d066fd4f9a8b201a3ddf346e880be 在 Windows 7 x64 上无法被终止的问题 [#3482]
- 修复 UseNewSymlinkResolver 导致应用程序同时创建链接和目标文件夹的问题 [#3481]
- 修复沙箱重命名导致组层次结构破坏的问题 [#3430]
- 修复配有红色框的加密沙箱阻止访问其根目录的问题 [#3475]
- 修复在 1.12.0 中引入的 SandMan 崩溃问题 [#3492]
...
- 将进程挂起/恢复移动到 SbieSvc [#3156]

### 修复
- 修复使用 AutoDelete 选项时 Microsoft Edge 的问题 [#post-3173507](https://www.wilderssecurity.com/threads/Sandybox-v1-12-1-pre-release.452939/#post-3173507)
- 修复与 Acrobat Reader 的警告问题： `SBIE2205 Service not implemented: CredEnumerateA` [#issuecomment-1826280016]
- 修复在沙箱化 Microsoft 365 应用（如 Outlook、Word)时打开任何网页链接时的 UNEXPECTED_KERNEL_MODE_TRAP BSoD [#3427]
- 修复强制进程警告消息的问题
- 修复在线更新器未尊重可携式模式的问题 [#3406]
- 修复在加密箱中无法使用快照功能的问题 [#3439]
...
- 修复 ThreadSuspendCount 在已终止线程上失败的问题 [25054d0] [#3375]
- 修复消息文本 [#3408]
- 修复 Firefox 120.0 的警告问题： `SBIE2205 Service not implemented: CredWriteA` [#3441]
- 修复 Firefox 120.0 的警告问题： `SBIE2205 Service not implemented: CredReadA` [#3453]
- 修复过时的 Chromium 路径问题 [#3436]
...
- 增加父母控制的故障排除脚本 [#3318]
- 开始对故障排除脚本的德语翻译 ...\SandboxiePlus\SandMan\Troubleshooting\lang_de.json [#3293]
- 增加 "get_cert SBIEX-XXXXX-XXXXX-XXXXX-XXXXX" 命令到 UpdUtil.exe，允许通过序列号使用命令行接收证书
- 增加撤销泄露或退款证书的机制
- 增加新的全局热键以将 SandMan 置于最上面 ALT+Break [#3320]
- 增加排除特定箱子不被 'Terminate all processes' 的选项 [#3108]
  - 注意：按下恐慌按钮热键三次，间隔少于 1 秒，即可终止所有进程而不例外
- 增加可自定义的全局热键，切换 "挂起强制程序" [#2441]
- 增加警告以防止广泛的 "强制文件夹" 设置 [#650]
- 增加 CheckExpectFile 函数到 Sandybox [#768]
...
- 修复自动更新器未提供版本更新的问题
- 修复新符号链接处理代码的问题 [#3340]
- 修复在非服务状态下 Scm_StartServiceCtrlDispatcherX 行为不当的问题 [#1246] [#3297]
- 修复使用 mklink 创建到 OpenPipePath 的符号链接原始文件夹的配置问题 [#3207]

### 删除
- 删除过时的 /nosbiectrl 开关 [#3391]
...
### 修改
- 改进 "Lock All Encrypted Boxes" 选项的行为 [#3350]
- 修复拼写错误：‘NoRestartOnPAC’ 选项变为 ‘NoRestartOnPCA’ [0e175ee]

### 修复
- 修复卸载受保护图像的问题 [#3347]
- 修复在 Sbie 消息选项卡中高亮无法关闭的问题 [#3338]
- 修复沙箱中的应用程序未通过 "开始菜单" 运行的问题 [#3334]
- 修复 NtQueryDirectoryObject 问题 [#3310]
- 修复一些 GUI 函数在 FileRootPath 值以 Volume{GUID} 开头时使用错误箱路径的问题 [#3345]
- 修复将程序保留在内存中时无法运行的问题 [#3349]
- 修复进程限制切换工作不正常的问题 [#3355]
...
### 新增
- 在沙箱选项窗口中增加容器头备份/恢复选项 [#post-3165085](https://www.wilderssecurity.com/threads/Sandybox-v1-11-3.452487/#post-3165085)

### 修改
- 更新至 7-Zip 23.01 [4ee1464]

### 修复
- 修复更改加密箱密码或导出加密箱时显示不正确的文本 [#3296]
- 修复图像文件未被创建为稀疏文件的问题
...
### 新增
- 增加个人证书过期时的更新警告
- 增加通过点击关闭按钮终止 UI 的选项 [#3253]
- 增加一键恢复被最小化到系统托盘的 Sandybox 窗口的选项 [#3251]
- 增加通过点击最小化按钮将 Sandybox 最小化到系统托盘的选项 [#3252]
- 增加独立的桌面集成设置 [#3246]
- 增加 AdGuard 浏览器助手的模板预设 [#3283]

### 更改
- 将托盘选项移至独立标签页
- 用更好的压缩图像替换了一些图形（感谢 idealths) [1af82bb]
- 将 /mount_protected 命令行开关的检查提前到 /mount 命令行开关之前 [#3289]

### 修复
- 修复了订阅证书识别问题
- 修复了关于窗口中 Logo 被截断的问题 [#3249]
- 修复了使用 RAM 驱动器时文件恢复的问题 [d82b62e]
...
### 新增
- 增加 'RamDiskLetter=R:\' 选项，允许将 RAM 驱动器根目录挂载到驱动器字母上 [938e0a8]

### 更改
- 将新选项布局更改为非复古视图的默认设置（可以在设置中更改回去) [94c3f5e]

### 修复
- 修复了重新创建 RAM 沙箱连接的问题 [2542351]
- 修复了设置向导中 Sandybox logo 缩放的问题 [#3227]
- 修复了在创建箱子向导中截断文本的问题 [#3226]
- 修复了与 ImBox.exe 的 Windows 7 兼容性问题 [1f0b2b7]
- 修复了 InjectDll/InjectDll64 设置未正确添加到沙箱配置中的错误 [#3254]
- 修复了与 'UseNewSymlinkResolver=y' 相关的问题 [01a2670]
- 修复了 Vintage View 模式下 SandMan 崩溃的问题 [#3264]
- 修复了固定多个指向同一文件的快捷方式的问题 [#3259]
...
### 更改
- 改善在线更新器代码
- 在箱子创建向导中将下拉列表替换为单选按钮 [#1381]

### 修复
- 修复在沙箱中创建的符号链接无法正常工作的错误 [#3181]
- 修复了 Plus UI 弹出窗口中文字截断的问题 [#3195]
...
### 修复
- 修复了 Sandybox-x64-v1.10.3 启动时崩溃的问题 [#3174]
- 修复了所有向导中的未翻译按钮问题 [#3133]
...
### 修复
- 修复了固定快捷方式的问题
- 修复了进程暂停/恢复上下文菜单的问题 [#3156]
- 修复与 Qt 基础安装程序相关的问题 [#2493] [#3153]
...
- "OpenClipboard=n" 现在也拒绝写入剪贴板 [#1367]

### 修复
- 修复跨重命名目录的问题
- 修复自动滚动不工作的情况 [#393]
- 修复与箱子删除保护有关的 UI 问题 [#3104]
- 修复链接参数处理中的问题 [#2969]
- 修复在 1.10.1 中引入的 IPC 问题 [#3132] [#3134]
- 修复固定运行条目图标的问题
...
### 新增
- 增加 UI 选项以选择自定义文本编辑器打开 Sandboxie.ini [#3116]
- 增加对箱子删除和内容删除的独立保护 [#3104]
- 在跟踪日志选项卡中增加“自动滚动”功能 [#393]

### 更改
- 重新设计 NT 对象句柄处理
- "OpenClipboard=n" 现在也在用户模式下实现，从而使其适用于隔离箱
- 更改 Delete V2 方案以使用 FilePaths.dat 中的驱动器字母（与 NT 路径的向后兼容性保留) [#3053]
- 改善快照合并的稳健性 [#3017]

### 修复
- 修复“禁用安全隔离”导致游戏停止播放音频的问题 [#2893]
- 修复 NtQueryDirectoryObject 未实现的问题 [#2734]
- 修复运行菜单条目的工作目录问题
- 修复沙箱命名空间中的全局符号链接不正确的问题 [#3112]
- 修复点击“显示堆栈跟踪”时出现的 'Addon already installed!' 错误 [#3114]
- 修复 BoxNameTitle=process.exe,- 在切换其他选项时被去除的问题 [#3106]
- 修复异步分配的 PCA 任务未被正确检测的问题 [#1919]
- 修复与第一个 Windows 10 版本不兼容的问题 [#3117]
- 修复移除沙箱仅在应用程序运行于沙箱时删除沙箱内容的问题 [#3118]
- 修复脚本引擎未正确终止引起的崩溃问题 [#3120]
- 修复 ImDisk 在 Sandybox 监督下导致 SBIE2337 和有时 BSoD 的问题 [#1092]
- 修复快照合并重复目录连接的问题 [#3016]
- 修复使用 Delete V2 重命名功能时与快照相关的问题
- 修复在使用网络共享时与 Delete V2 相关的问题
- 修复在使用 "UseVolumeSerialNumbers=y" 时访问驱动器根的问题
- 修复移除快照在使用 Delete V2 时复活已删除文件的问题 [#3015]

### 删除
- 删除对 Internet Download Manager 的模板支持 [#991]
...
- 在 SbieCtrl.exe 中增加 `What's new in Sandybox` 对话框，以突出 Plus UI 的新功能
  - 注意：该对话框在安装 Sandybox Classic 后显示
- 增加 "fixdacls" 命令到 KmdUtil.exe，用于修复 Sandybox 文件夹上损坏的 DACL 条目，解决 SbieDll.dll 加载失败的问题
- 增加隐藏 Sandybox 自身进程的功能 [#3054]
- 增加在 Plus UI 中缓存 Sandybox 消息的功能 [#2920]
- 增加直接从 SBIE 消息弹出窗口调用故障排除向导的按钮

### 更改
- 安装向导现在有一个专门的更新配置页面
  - 注意：对所有未启用更新的用户只会显示此页面一次
- 将支持页面拆分为 Sandybox Support 和 Sandybox Updater 标签
...
### 修复
- 修复 Sandybox Classic 安装程序中的卸载问题 [d1863ff]
- 引入 Chrome 在 Windows 11 的 KB5027231 更新后无法启动的解决方法 [#3040]
- 改善调试版的 procmon/堆栈跟踪兼容性
- 修复非标准命令行的问题
- 修复在线更新器以每天而不是每周间隔进行检查的问题

### 删除
- 清理了重复代码（感谢 lmou523) [#3067]
...
### 更改
- 改善 Sandybox Classic 安装程序中的卸载过程（感谢 sredna) [#3044]

### 修复
- 修复当路径是通过符号链接创建时出现的链接错误（感谢 lmou523) [#3038]
- 修复文件覆盖对话框后恢复窗口的焦点 [#3048]
- 修复了由于异常代码 c0000005 导致的 Sandybox 崩溃问题 [#3024]
...
### 新增
- 增加可配置的工具栏项目（感谢 typpos) [#2961]
- 增加 Plus UI 的新图标（感谢 typpos) [#3010] [#3011]

### 更改
- 对 BlockSoftwareUpdaters 模板进行小幅更新（感谢 APMichael) [#2991]

### 修复
- 修复在 Plus UI 中使用 “复制面板” 时出现的空行问题 [#2995]
- 修复 Plus 安装程序在更新当前已安装版本时未遵循 IniPath 值的问题（感谢 offhub) [#2994]
- 修复崩溃转储的时间戳
- 修复 Firefox 114 在启用了启动限制的安全强化箱中崩溃的问题（感谢 offhub) [#3019]
- 修复当没有 GUI 代理时的错误（感谢 lmou523) [#3020]

## [1.9.6 / 5.64.6] - 2023-05-30

### 新增
- 为所有跟踪消息添加完整堆栈跟踪
  - 激活 Trace Log 选项卡还默认开启 Keep Terminated 功能 [#2988]
- 新增以非管理员身份启动默认记事本编辑器的选项 [#2987]

### 修复
- 修复了 SbieDrv 驱动程序中的令牌操作问题
- 修复“重置所有 GUI 选项”未按预期重置所有 GUI 部分的问题 [#2967]
- 修复 Box Type 预设下的 sbie:// 链接 [#2959]
- 修复“重置所有 GUI 选项”导致所有沙箱名称消失 [#2972]
- 修复游戏无法通过“从开始菜单运行”正常启动的问题 [#2969]
- 修复 Microsoft Excel 数据网格中的拖放问题 [9455e96]
  - 可在 [#856] 中提供其他拖放场景的反馈
- 修复回归：DLL 加载问题（未找到入口点) [#2980]
- 修复在虚拟化方案 v2 激活时 Sandybox 未标记已删除文件或注册表项的问题（1.9.4/1.9.5) [#2984]
- 修复 SandMan：Sandboxie.ini 的时间戳 [#2985]
- 修复多次按下“显示 NT 对象树”按钮后崩溃（Plus UI) [#2943]

## [1.9.5 / 5.64.5] - 2023-05-26

### 新增
- 新增将 SBIE 消息 ID 转换为可点击链接 [#2953]
- 新增相同日志消息数量的限制 [#2856]

### 修复
- 修复 sbie:// 链接无法正常工作的问题 [#2959]
- 修复 Sbie 消息、Trace Log、Recovery Log 选项卡不应隐藏某些列的问题 [#2940]
- 修复 RegisterDeviceNotificationW 函数 DBT_DEVTYP_DEVICEINTERFACE [#2129] [#1975]
- 修复可能在 1.9.4 中引入的 BSoD 问题（由 7starsseeker 报告) [#2958]
...
### 变更
- 改善隐私增强的盒子类型的兼容性模板（感谢 offhub) [#2899]
- 改善设置中的支持页面和提醒 [#2896]
- 改善签名错误消息 [#2931]
- 更改在存在例外时不显示“No Inet” [#2919]

### 修复
- 修复 ARM64 构建中的 Qt6 问题
- 修复无驱动器字母的盒子挂载时的 V2 删除问题
- 修复高 DPI 缩放下的图标覆盖问题
- 修复多项选择中的行为问题（感谢 okrc) [#2903]
- 修复默认盒子分组的问题（感谢 okrc) [#2910]
- 修复沙箱重命名的问题 [#2912]
- 修复打开系统保护存储的复选框未选中的问题 [#2866]
- 修复防火墙规则缺乏色彩对比使文本难以阅读的问题 [#2900]
- 修复RecoverFolder显示 GUID 而不是文件夹名称的问题 [#2918]
- 修复操作停止时沙盘图标覆盖在系统托盘中卡住的问题 [#2869]
- 修复文件面板不允许永久调整列大小的问题 [#2930]
- 修复重命名具有沙箱运行条目的盒子可能会破坏这些条目的问题 [#2921]
- 修复设置向导后 WFP 未启用及其他问题 [#2915]
- 修复沙箱视图中的名称列永远不应被隐藏 [#2933]
- 修复文件面板未共享沙箱视图列中的右键选项的问题 [#2934]
- 修复 NetworkEnableWFP 和 EnableObjectFiltering 设置复选框的问题 [#2935]
- 修复未引用的服务路径 [#537]
- 修复在沙箱中运行 Microsoft Word 的插入图表失败 [#2863]
- 修复切换沙箱类型预设配置时发生错误的问题 [#2941]
- 修复无法打开 Microsoft Excel 的问题（感谢 lmou523) [#2890]
...
- 新增指定 Sandboxie.ini 位置到注册表的功能
  - 打开 "HKLM\SYSTEM\CurrentControlSet\Services\SbieDrv" 并设置 "IniPath" [#2837]
...
### 修复
- 修复命令行问题 [#2858]
- 修复恢复窗口中总是显示在最上面的问题 [#2885]

## [1.9.2 / 5.64.2] - 2023-04-24

### 新增
- 新增盒子预设菜单选项以切换 'DisableForceRules=y'（感谢 offhub) [#2851]
...
### 修复
- 修复经典安装程序中的系统检查 [#2812]
- 修复 Normal[File/Key]Path 的内存损坏问题 [#2588]
- 修复快照管理器窗口中的 DPI 缩放问题 [#782]
- 修复 arm64 DialogProc 钩子的问题 [#2838]
- 修复与 7-Zip 文件系统实现的 Qt6 兼容性问题 [#2681]
- 修复使用 Qt6 的 TLS 支持问题 [#2682]
- 新增 Spotify 的模板预设 [#2673]
- 新增查找器的缺失本地化 [#2845]
...
- 修复更新程序问题 [#2790]
- 修复新盒子向导中的盒子命名问题，改为使盒子名称固定 [#2792]
- 修复 Firefox/Thunderbird 的问题 [#2799]
...
- 修复长期存在的与隔离类型盒子有关的 ping 问题 [#1608]

## [1.8.2 / 5.63.2] - 2023-04-01

### 新增
- 重新设计 CreateAppContainerToken 钩子以返回受限令牌，用于解决该问题 [#2762]
...
### 修复
- 修复全局设置 INI 部分编辑的问题
- 修复 'UseRegDeleteV2=y' 的问题 [#2756]
- 自动运行路径现在支持任何长度 [#2769]（感谢 Sapour)
- 恢复窗口：未选择任何文件/文件夹时不再显示删除确认对话框 [#2771]（感谢 Sapour)
- 修复 WeChat 崩溃问题 [#2772]
...
### 修复
- 修复了跟踪日志中查找器搜索的问题
- 修复了 NT 名称空间虚拟化的问题 [#2701]
- 修复了“以未沙盒模式运行”的问题 [#2710]
- 修复了新盒向导的问题 [#2729]
- 修复了突破进程和用户限制沙箱的问题 [#2732]
- 修复了一些 UI 问题 [#2733]
- 修复了无用的帮助按钮 [#2748]
...
- 添加了 CreateAppContainerToken 的钩子，这也应该改善与其他应用程序的兼容性 [#1926]
...
- 修复了没有系统令牌启动服务的问题
- 修复了与新文件迁移设置相关的问题 [#2700]
- 修复了 ARM64 上的外壳集成问题 [#2685]
- 修复了与驱动程序验证器的新问题 [#2708]
...
- 避免在拖动时阻止 Windows 资源管理器 [#2660]
- 修复了 QtSingleApp 的问题 [#2659]
- 修复了更新程序有时未能创建临时目录的问题 [#2615]
- 修复了快照移除的问题 [#2663]
- 修复了符号链接的问题 [#2606]
...
### 修复
- 修复了 BlockNetworkFiles=y 与 RestrictDevices=y 不一起工作的情况 [#2629]
- 修复了因 1.7.0 引入的 SandMan 崩溃问题
- 修复了跟踪日志过滤器区分大小写的问题
- 修复了与 Delete V2 相关的性能问题
- 修复了 NtQueryDirectoryFile 数据对齐的问题 [#2443]
- 修复了 Microsoft Edge 111 开发版的问题 [#2631]
- 修复了 mio 套接字的问题 [#2617]
- 修复了从选项/设置窗口创建的运行菜单条目的问题 [#2610]
- 修复了使用快照时开始菜单的问题 [#2589]

## [1.7.0 / 5.62.0] - 2023-01-27

### 新增
- 添加了 OnFileRecovery 触发器，允许在恢复文件之前进行检查 [#2202]
- 向沙箱选项添加了更多预设
  - 注意：这些只能在盒子为空时更改
- 添加了带有附加设置的新文件迁移选项页面
...
### 修复
- 解决了 SbieDll.dll 与影子堆栈的不兼容性，并启用了 /CETCOMPAT 对 SbieDll.dll [#2559]
...
### 新增
- 向 Sandybox Classic 添加了应用支持者证书的选项

### 更改
- 有限时间证书现在延长了 1 个月的有效期，以改善续订体验

### 修复
- 修复了希伯来语语言（经典 UI)的问题 [#2608]
- 修复了开始菜单集成和快照的问题 [#2589]
...
### 修复
- 修复了 Microsoft Edge 在安全加固盒中的问题 [#2571]
- 在 ARM64 系统上打开了 OpenIpcPath=\\{BEC19D6F-D7B2-41A8-860C-8787BB964F2D}
- 修复了与 Windows 11 21H2 ARM64 不兼容的问题 [#2431]

## [1.6.4 / 5.61.4] - 2022-12-31

### 新增
- 添加选项以禁用启动时沙箱清理 [#2553]
- 在帮助菜单中添加了适用于 Plus 和 Classic UI 的贡献指南 [#2551]

### 修复
- 修复了 SandMan 启动时自动删除沙箱的问题 [#2555]
- 修复了 Windows 11 上的上下文菜单问题，当 SandMan 尚未运行时 [#2284]
- 修复了关于 cmbDefault 的显示问题 [#2560] (okrc)
- 修复了色彩不准确的问题 [#2570] (okrc)
- 修复了第一次启动时 Templates.ini 加载的问题 [#2574]
...
- 修复了 32 位 Windows 10 系统上的 WoW64 thunking 问题 [#2546]
- 修复了沙箱中自动删除选项未工作的情况 [#2531]
  - 注意：现在每次重新启动后都会进行清理
- 修复了 Windows 11 下网络位置感知的问题 [#2530]
- 修复了恢复窗口的问题 [#2458]
...
### 新增
- 添加腾讯 TIM 的模板 [#2516] (感谢 TooYoungTooSimp)
...
### 修复
- 修复了支持设置下拉菜单重复出现的问题 [#2502] (okrc)
- 将翻译文件夹标记为非推荐使用，将在安装时被删除 [#2500] (lufog)
- 修复了未计算隐藏项目磁盘使用的问题 [#2503] (okrc)
- 修复了可按进程设置的布尔设置问题 [#2495]
- 修复了未计算隐藏项目磁盘使用的问题 [#2503]
...
- 新增更新机制，允许增量更新
  - 用于“实时”更新频道中的夜间测试构建
  - 用于安装后更新 Templates.ini 和翻译文件 [#1105]
...
### 修复
- 修复了回收图标未及时更新的问题 [#2457] (感谢 okrc)
- 修复了“索引超出范围”的问题 [#2470] (感谢 okrc)
- 修复了安装后启动 SandMan 的问题 [#2284]
- 修复了 my_version.h 的编码问题 [#2475]
- 修复了空组闪烁序列的问题 [#2486] (感谢 okrc)
...
- 在 Plus UI 上新增越南语 [#2438]

### 修复
- 修复了沙箱选项的问题 [#2400]
- 修复了 Smart App Control 的问题 [#2341]
- 修复了使用隐私沙箱时快照的问题 [#2427]
- 修复了 m_pColorslider 更改未应用的问题 [#2433] (感谢 okrc)
- 修复了当文件面板打开时切换快照的问题
- 修复了选择空沙箱时文件面板的问题 [#2419]
- 修复了菜单图标未禁用的问题 [#2406]
- 修复了与 Microsoft Edge 的兼容性问题 [#2312]
- 修复了 UsePrivacyMode=y 与 Windows 7 的兼容性问题 [#2423]
- 修复了 Sandybox 卸载时的一个小问题 [#2421]
- 修复了驱动初始化失败引起的 BSoD 问题（在 1.5.1 中引入) [#2431]
- 修复了即使真实路径不存在也会列出虚假路径的问题 [#2403]
- 修复了 Firefox 106.x 请求对插件可执行文件的写入访问问题 [#2391] [#2411]
...
- 修复了与 1.5.0 引入的本机 32 位 Windows 的问题 [#2401]
...
### 修复
- 修复了使用自定义路径重命名沙箱时的问题 [#2368]
- 正确修复了 Firefox 106 的问题 [46e9979]
- 修复了替代 UI 模式的问题 [#2380]
- 修复了与突围进程的命令行损坏的问题 [#2377]
- 修复了与隐私增强沙箱类型的问题 [#2342]
- 修复了沙箱对象目录初始化的问题 [#2342]
- Sandybox 不再留下永久的目录对象
- 修复了安全问题 ID-21 AlpcConnectPortEx 未被驱动筛选 [#2396]
- 修复了程序控制选项的问题 [#2395]
...
### 新增
- 支持 Windows ARM64 [#1321] [#645]
  - 移植 SbieDrv 为 ARM64
  - 移植低级注入机制为 ARM64/ARM64EC
  - 移植系统调用钩子为 ARM64/ARM64EC
  - 移植 SbieDll.dll 为 ARM64/ARM64EC
  - 注意：ARM32 在 ARM64 上未实现，将终止并显示消息 SBIE2338
  - 注意：当 Sandybox 正在运行时，它会全局禁用对 x86 进程的 CHPE 二进制文件的使用 - 这是强制进程功能所需的。这可以通过将全局选项 "DisableCHPE=n" 添加到 Sandboxie.ini 来禁用，这将终止在沙箱外启动的 x86 进程，并显示消息 SBIE2338，而不是强制执行
...
- 编辑模板时避免窗口重叠 [#2339] (感谢 okrc)
- 修复了 OpenWinClass UI 设置的错误写入 [#2347] (感谢 okrc)
- 修复了本地模板的问题 [#2338] (感谢 okrc)
- 修复了与 Edge WebView2 的兼容性问题 [#2350]
- 为 Firefox 106 内容进程沙箱问题添加临时解决方法
- 修复了重命名沙箱的问题 [#2358] (感谢 okrc)
...
### 新增
- 为资源访问模式新增工具提示 [#2300]
- 新增控制 ApplyElevateCreateProcessFix 的 UI 选项 [#2302]
...
- 修复了突围进程页面中的按钮标题错误
- 修复了保存沙箱恢复选项的问题
- 修复了 SandMan 的显示问题 [#2306] (感谢 okrc)
- 修复了主题未自动更改的问题 [#2307]
- 修复了在 Windows 关机时保存 SandMan 窗口状态的问题
- 修复了其他小问题 [#2301]
- 修复了 106.x 中引入的与 Microsoft Edge 的问题 [#2325]
- 修复了 Vivaldi hooking 问题，UseVivaldiWorkaround 现在不再需要 [#1783]
- 修复了高级选项页面上杂项选项卡的问题 [#2315]
...
### 修复
- 修复了 Plus UI 中菜单栏的问题 [#2280] (感谢 okrc)
...
### 新增
- 为 Windows 11 Shell 菜单新增本地化 [#2229]

### 更改
- 改进了 Plus UI 中恢复窗口的行为 [#2266]

### 修复
- 修复了 Sandboxie-Plus.ini 中的过期数据问题 [#2248] (感谢 okrc)
- 修复了虚假清单的问题 [#2252]
- 修复了 XYplorer 的问题 [#2230]
- 修复了 Plus UI 中的崩溃问题 [e9e21c2]
- 修复了当 NoIcons=1 时 m_pCleanUpButton 显示为空的问题 [#2273] (感谢 okrc)

## [1.3.4 / 5.58.4] - 2022-09-19

### 添加
- 在 Plus UI 中添加了 NoRenameWinClass
- 将 Windows.UI.* 添加到硬编码的已知类列表中，以解决 WinUI 应用程序的问题 [#2109]

### 更改
- NoRenameWinClass 现在支持通配符

### 修复
- 修复了启动时未检测到默认箱的问题 [#2195]
- 修复了移动沙盒菜单的问题 [#2225]（感谢 okrc)
- 修复了 Sandboxie-Plus.ini 中过期数据的问题 [#2234]（感谢 okrc)
- 修复了自动启动问题 [#2219]
- 修复了防火墙 UI 问题，所有程序条目缺少 * 前缀 [#2247]
- 修复了 BlockPorts 模板中缺少 * 前缀的问题 [4420ba4]
- 修复了与各种 Electron 应用程序相关的问题 [#2217] [#2235] [#2201] [#2166]
...
### 添加
- 当 LogFile 注册设置应用作为解决方法时，添加了 domain\user 表示法 [#2207]
  - 用法：在 "HKLM\SYSTEM\CurrentControlSet\Services\SbieSvc" 中添加 REG_SZ "LogFile"，值为 "3;[path]\Sandybox.log"
- 添加了阻止主机进程访问沙盒进程的选项 [#2132]
  - 用法：DenyHostAccess=Program.exe,y
  - 注意：默认情况下，该保护仅适用于写访问，也就是说，未沙盒进程仍然可以获得只读访问权限
  - 若要防止主机进程获得读取权限，还必须设置 ConfidentialBox=y，这需要支持者证书
- 为 ReHIPS 添加了兼容性模板
- 添加了在隐私箱类型中创建所有默认文件夹的功能 [#2218]

### 更改
- 改进了非管理员用户的 SandMan 设置行为 [#2123]
...
- 添加了独立于暗黑主题切换融合主题的能力
- 添加了从支持页下载更新的能力
- 将缺失的系统调用添加到增强版箱类型 [88bc06a] [b775264] [04b2377]（感谢 Mr.X)
- 在 Plus UI 设置和箱选项对话框中添加了搜索框 [#2134]
- 将韩语翻译添加到 Plus UI [#2133]（感谢 VenusGirl)
- 将沙盒管理器（SandMan)托盘菜单添加了分组功能 [#2148]
...
### 修复
- 修复了显示沙盒配置的问题 [#2111]
- 修复了在切换视图时出现的闪烁问题 [#2050]
- 修复了 Plus UI 中各种复选框的不一致问题 [ef4ac1b] [06c89e3]
- 修复了证书验证问题 [238cb44]
- 修复了 "UseRuleSpecificity" 设置的问题 [#2124] [file.c#L965-L966]
...
### 添加
- 在所有列表中添加了交替行颜色的选项 [#2073]

### 更改
- SandboxieLogon 默认情况下现在禁用，因为它与第三方恶意软件工具不兼容 [#2025]
- 访问视图列表现在会在文件和键路径末尾添加尾部 "*"，与驱动程序方式相同 [2039]
- Shell 集成的设置现在由 SandMan 完成，而不是安装程序
- 卸载程序现在可以删除沙盒文件夹 [#1235]
...
- 修复了新钩子机制的问题
- 修复了 1.2.0 中引入的与 Win32k 钩子相关的蓝屏问题 [#2035]
- 修复了 Element 1.11 和 Electron 解决方法的问题 [#2023]
...
- 修复了软件兼容性选项卡的问题
- 修复了在构建 1.1.1 中引入的 OpenKeyPath 问题 [#2006]
...
### 更改
- 重新构建了钩子管理，已卸载的 DLL 现在可以正确取消钩住 [#1243]
...
- 恢复了 Plus UI 的土耳其语翻译 [#1419]（感谢 fmbxnary)

### 修复
- 修复了删除时恢复窗口的问题 [#1948]
- 修复了路径列的双击问题 [#1951]
- “AllowBoxedJobs=n” 恢复为默认行为，因为已报告问题 [#1954]
- 修复了互联网阻止的问题 [#1955]
- 修复了 Plus UI 中的分组问题 [#1950]
- 修复了在 Windows 11 中使用 Win32k 钩子时 CredentialUIBroker.exe 的问题 [#1839]
- 修复了 Delete V2 的问题 [#1939]


## [1.1.2 / 5.56.2] - 2022-06-14

### 新增
- 将缺失的文件恢复日志添加到 SandMan [#425]
- 当所有文件恢复完毕时，立即恢复窗口将自动关闭 [#1498]
- SandMan 的立即恢复窗口默认为总在最上方，类似于 SbieCtrl；可通过 “Options/RecoveryOnTop=n” 禁用 [#1465]
- 增加了从预设子菜单切换立即恢复的选项 [#1653]
...
### 修复
- 修复了在 1.1.1 中引入的不必要的 Sandybox 配置重载问题 [#1938]
- 修复了恢复窗口焦点的问题 [#1374]
- 修复了在 1.1.1 中引入的桌面对象问题 [#1934]
- 修复了使用 GPO 预设时 Edge 启动提升的问题 [#1913]


## [1.1.1 / 5.56.1] - 2022-06-07

### 新增
- 兼容性模板现在可以从设置窗口查看 [#1891]
- 刷新命令现在绑定到 F5 [#1885]
- 增加了更多首次启动向导选项
- 增加了一个选项以在任何给定盒子打开时永久禁用立即恢复 [#1478]
- 双击路径列现在会在 Windows Explorer 中打开盒子根目录 [#1924]

### 更改
- 更改了移动盒子的行为 [#1879]
- 改进了 PreferExternalManifest 选项的实现
- Win32k 钩子现在默认仅用于 Edge 和 Chromium 应用程序，因为它们会导致其他软件出现问题 [#1902] [#1912] [#1897]
- “AllowBoxedJobs=y” 现在为默认行为

### 修复
- 修复了 Windows 11 上与 KB5014019 相关的 Edge 问题
- 修复了在使用 “SeparateUserFolders=y” 时与新 Delete V2 机制相关的问题 [#1885]
- 修复了凭证问题 [#1770]
- 修复了强制进程优先级 [#1883]
- 修复了与新 Delete V2 机制相关的问题
- 修复了在较旧版本的 Windows 上的 Windows 11 菜单的问题 [#1877]
- 刷新现在在没有 WatchBoxSize 选项的情况下也能正常工作 [#1885]
- 修复了 WatchBoxSize=true 导致的崩溃问题 [#1885]
- 修复了恢复文件夹路径的问题 [#1840]
- 修复了影响 Acrobat Reader 的 Sbie 桌面和 wndStation 问题 [#1863]
- 修复了盒子分组问题 [#1921] [#1920]
- 修复了 BreakoutFolder 的问题 [#1908]
- 修复了 x86 异常处理的 SbieDll.dll 问题
- 修复了影响 Visual Studio 的应用程序特定的注册表 hive (RegLoadAppKey) 问题 [#1576] [#1452] [#205]
- 增加了盒子大小信息 [#1780]

### 更改
- 重新构建了 Sandybox 用于标记主机文件为已删除的机制
...
### 修复
- 修复了文件夹重命名问题（这需要 UseFileDeleteV2=y) [#71]
- 修复了进程访问的问题 [#1603]
- 修复了翻译问题 [#1864]
- 修复了盒子选择窗口的 UI 问题 [#1867]
- 修复了切换语言时的 UI 问题 [#1871]


## [1.0.22 / 5.55.22] - 2022-05-15

### 新增
- 增加了 SandMan.exe 的自动更新下载和静默安装选项 [#917]
- 跟踪监视模式现在可以保存到文件 [#1851]
- 跟踪日志现在显示 IPC 对象类型信息
- 增加了对 Windows 11 上下文菜单的支持

### 修复
- 修复了 SandMan 崩溃问题 [#1846]
...
### 修复
- 修复了使用 MinGW 工具链编译的 Firefox/Chromium 浏览器的问题 [#538]
- 修复了 Plus UI 上的文件夹恢复问题 [#1840] [#1380]


## [1.0.20 / 5.55.20] - 2022-05-02

### 修复
- 修复了在上一版本中引入的 Firefox 视频播放问题 [#1831]
- 修复了与驱动程序相关的蓝屏死机问题 [#1811]
- 修复了编辑启动限制条目的问题
- 修复了网络选项卡的问题 [#1825]
- 修复了如果 SandMan 以管理员身份运行的便携模式问题 [#1764]
...
### 新增
- 增加了对组的拖放支持 [#1775]
- 增加了对所有条目类型的删除键支持 [#1779]
- 当尝试在打开了 COM 的盒子中运行 explorer.exe 时给予警告 [#1716]

### 修复
- 修复了 SandMan UI 中的崩溃问题 [#1772]
- 修复了在启用 EnableObjectFiltering 的情况下某些安装程序的问题 [#1795]
- 修复了允许 NtCreateSymbolicLinkObject 在沙箱中安全使用
- 增加了对 Vivaldi 钩子问题的解决方法 [#1783]
  - 注意：该修复是临时的，可以通过 UseVivaldiWorkaround=n 禁用
- 修复了与快照相关的注册表问题 [#1782]
- 修复了盒子分组问题 [#1778] [#1777] [#1776]
- 修复了与盒子分组相关的进一步问题 [#1698] [#1697]
- 修复了快照 UI 的问题 [#1696] [#1695]
- 修复了恢复对话框焦点的问题 [#1374]
...
### 新增
- 将小型浏览器添加到 BlockSoftwareUpdaters 模板中（由 APMichael 提供) [#1784]

### 更改
- 默认情况下，失败的内存读取尝试对未沙箱化的进程不再导致消息 2111
  - 注意：仍然可以通过设置中的 “NotifyProcessAccessDenied=y” 来启用该消息
- 重新排序了 BlockSoftwareUpdaters 模板（由 APMichael 提供) [#1785]

### 修复
- 修复了在隔室模式中管道模拟的问题
- 修复了近期构建中引入的盒子清理问题
- 修复了丢失的跟踪日志清理命令 [#1773]
- 修复了无法取消固定已钉到运行菜单的程序的问题 [#1694]
...
- 修复了与 HideOtherBoxes=y 相关的回归问题 [#1743] [#1666]
...
### 新增
- 增加通知以警告默认更新检查器落后于 GitHub 上的最新版本，以确保只有无缺陷的构建被提供为更新 [#1682]
- 将主要浏览器添加到 BlockSoftwareUpdaters 模板中（由 Dyras 提供) [#1630]
- 当 Sandboxie-Plus.ini 不可写时给予警告 [#1681]
- 增加了对关键区域的清理（由 chunyou128 提供) [#1686]

### 更改
- 改进了突破进程的命令行处理 [#1655]
- 禁用 SBIE2193 通知（由 isaak654 提供) [#1690]
- 改进了错误信息 6004 [#1719]

### 修复
- 修复了新托盘列表的暗黑模式问题
- 修复了当 Sandboxie-Plus.ini 不可写时未显示警告的问题 [#1681]
- 修复了软件兼容性复选框的问题（感谢 MitchCapper) [#1678]
- 修复了在盒子关闭时事件并不总是执行的问题 [#1658]
- 修复了 key_merge.c 中的内存泄漏
- 修复了隐私模式下枚举注册表键的问题
- 修复了在 1.0.13 中引入的设置问题 [#1684]
- 修复了解析防火墙端口选项时的崩溃问题
- 修复的安全问题 ID-19：在某些情况下，沙箱进程可能会获得对未沙箱化线程的写权限句柄 [#1714]
...
### 更改
- 禁用新沙箱中的 Chrome 和 Firefox 钓鱼条目（由 isaak654 提供) [#1616]
- 更新了 BlockSoftwareUpdaters 模板中的 Mozilla 路径（由 isaak654 提供) [#1623]
- 将 “暂停强制程序规则” 命令重命名为 “暂停强制程序”（仅限 Plus)
- 重新构建托盘图标生成，现在使用叠加图标，增加了忙碌叠加

### 修复
- 修复了在隐私模式下访问网络驱动器的问题 [#1617]
- 修复了在隔室模式下 ping 的问题 [#1608]
- 修复了在盒子中创建和关闭大量进程时 SandMan UI 冻结的问题 [#1607]
- 修复了在运行菜单中编辑命令行条目未被识别的问题 [#1648]
- 修复了驱动程序中的蓝屏问题（感谢 Diversenok)
- 修复了与 Windows 11 Insider 构建 22563.1 的不兼容问题 [#1654]
...
- 当打开未沙盒的 regedit 时添加警告 [#1606]
- 添加了在官方 Qt 5.15.2 中缺少的语言文件（由 DevSplash 提供) [#1605]
...
- 将 Listary 模板更新到 v6（由 isaak654 提供) [#1610]

### 修复
- 修复了与 SECUROM 的兼容性问题 [#1597]
- 修复了模态性问题 [#1615]
- 修复了 Templates.ini 中 OpenWinClass 的特殊形式 [d6d9588]

## [1.0.10 / 5.55.10] - 2022-02-06

### 新增
- 添加了仅在托盘中显示正在运行的盒子的选项 [#1186]
  - 额外选项仅显示固定的盒子，在盒子选项中可以将盒子设置为始终显示在托盘列表中（固定)
- 添加了重置 GUI 的选项菜单命令 [#1589]
- 添加了“以未沙盒方式运行”上下文菜单选项
- 添加了新的触发器 "OnBoxDelete"，允许在盒子内容删除之前指定一个在未沙盒状态下运行的命令
  - 注意：这可以用作 "DeleteCommand" 的替代 [#591]
- 选定的盒子操作（删除)不再显示进度对话框 [#1061]
  - 如果正在运行操作的盒子显示闪烁的沙漏图标，可以使用上下文菜单取消该操作

### 更改
- 现在可以使用 "HideHostProcess=program.exe" 来隐藏 Sandybox 服务 [#1336]
...
- 修复了重新打开已打开窗口的问题 [#1584]
- 修复了桌面访问问题 [#1588]
- 修复了命令行调用处理的问题 [#1133]
- 修复了在切换总是在最上面属性时主窗口状态的 UI 问题 [#1169]
- 修复了托盘列表中盒子上下文菜单的问题 [1106]
- 修复了 "AutoExec=..."
- 修复了取消盒子删除操作不工作的问题 [#1061]
- 修复了 DPI 缩放和颜色选择器对话框的问题 [#803]
...
- 添加了版本号到 Sandybox Classic 的标题栏
- 添加了返回到空盒状态的选项，同时保留所有快照
- Sandboxie-Plus.ini 现在可以放置在 C:\ProgramData\Sandybox\ 文件夹中并优先使用（用于商业用途)
- 添加对 AF_UNIX 在 Windows 上的支持，以解决与 OpenJDK17 及后续版本相关的问题 [#1009] [#1520] [#1521]

### 更改
- 重新设计了突破机制，使其基于服务并不允许父进程访问被突破的子进程
- 启用沙盒进程的目录连接创建 [#1375]
- 在盒子创建时恢复 "AutoRecover=y" [#1554]
- 改进了快照支持 [#1220]
...
- 修复了进度对话框的问题 [#1562]
- 修复了在 Sandybox 中处理目录连接的问题 [#1396]
- 修复了 File_NtCloseImpl 中的句柄泄漏
- 修复了在上一个版本中引入的最大化窗口的边框问题 [#1561]
- 修复了一些索引溢出的问题（感谢 7eRoM) [#1571]
- 修复了在通过上下文菜单运行沙盒时启动 SandMan 的问题 [#1579]
- 修复了在创建主窗口时的暗色模式闪烁问题 [#1231]
- 修复了与快照错误处理相关的问题 [#350]
...
### 新增
- 在 Plus UI 中添加了葡萄牙语（葡萄牙)支持（由 JNylson、isaak654、mpheath 提供) [#1497]
- 添加了 "BreakoutProcess=program.exe"，通过此选项可以从盒内启动未沙盒的应用程序 [#1500]
  - 程序图像必须位于沙盒外部才能正常工作
...
- 为 Classic 安装程序添加了静默卸载开关 `/remove /S`（由 sredna 提供) [#1532]

### 更改
- 文件名 "sandman_pt" 改为 "sandman_pt_BR"（巴西葡萄牙语) [#1497]
- 文件名 "sandman_ua" 改为 "sandman_uk"（乌克兰语) [#1527]
- 更新了 Firefox 更新阻止器（由 isaak654 发现) [#1545]

### 修复
- 修复了打开所有文件访问 OpenFilePath=* 的问题 [#971]
- 修复了打开网络共享的相关问题 [#1529]
- 修复了 Classic 安装程序的潜在升级问题（由 isaak654 提供) [130c43a]
- 修复了 Classic 安装程序的小问题（由 sredna 提供) [#1533]
- 修复了 Ldr_FixImagePath_2 的问题 [#1507]
- 使用 SandMan UI 和 UI 关闭时，使用 "Run Sandboxed" 将保持关闭
- 修复了 Util_GetProcessPidByName 的问题，这应该解决驱动程序有时在启动时未能启动的问题 [#1451]
- SandMan 现在在启动被包装的进程时像 SbieCtrl 一样在后台运行 [post506](https://forum.xanasoft.com/threads/direct-start-firefox-in-box.173/post-506)
- 修复了全屏时持久盒边框未显示任务栏的问题 [post474](https://forum.xanasoft.com/threads/taskbar-problems-when-using-Sandybox-in-windows-11.224/post-474)
- 修复了盒边框未跨多个显示器的问题 [#1512]
- 修复了使用 DPI 缩放时边框的问题 [#1506]
- 修复了 Qt 的 DPI 问题 [#1368]
- 修复了在暗色模式下窗口创建时的亮闪问题 [#1231]
- 修复了 PortableRootDir 设置的相关问题 [#1509]
- 修复了在驱动未连接时设置窗口崩溃的问题
- 修复了 Finder Tool 的 DPI 问题 [#912]
- 修复了 reused 进程 ID 的另一个问题 [#1547]
- 修复了与 SeAccessCheckByType 相关的问题 [#1548]
...
- 添加了在 Plus UI 中设置自定义 INI 编辑器的选项 [#1475]
- 添加了选项 "LingerLeniency=n" 以解决问题 [#997]

### 更改
- 重新设计了驱动中的系统调用调用代码
  - Win32k 钩子现在与 HVCI 兼容 [#1483]

### 修复
- 修复了驱动中的内存泄漏（conf_user.c)
- 修复了在打开路径中重命名文件的问题，该问题在 1.0.6 中引入
- 修复了导致 Chromium 浏览器无法正确关闭的问题 [#1496]
- 修复了与 Start.exe 相关的问题 [#1517] [#1516]
- 修复了 SandMan 中 reused 进程 ID 的问题
- 修复了 KmdUtil 有时未正常终止驱动的情况 [#1493]
...
### 新增
- 将“打开方式”替换为 Sandybox 对话框，以适应 Windows 10 [#1138]
...
- 在恢复窗口中添加了双击恢复文件和文件夹的功能 [#1466]
- 在 Plus UI 中添加了乌克兰语支持（由 SuperMaxusa 提供) [#1488]

### 更改
- "UseSbieWndStation=y" 现在是默认行为 [#1442]
- 由于不兼容（蓝屏死机)，在启用 HVCI 时禁用了 Win32k 钩子 [#1483]

### 修复
- 修复了隐私模式下盒子初始化的问题 [#1469]
- 修复了近期版本中引入的快捷方式创建问题 [#1471]
- 修复了隐私增强型盒子和规则特异性中的各种问题
- 修复了与 SeAccessCheckByType 等相关的问题
- 修复了在 32 位 Windows 上的 Win32k 钩子相关问题 [#1479]
...
- 添加对 Osiris 和 Slimjet 浏览器的模板支持（由 Dyras 提供) [#1454]

### 更改
- 改进了 SbieDll 初始化
- 将 Name_Buffer_Depth 的大小加倍 [#1342]
- 改进了模板视图中的文本过滤器 [#1456]

### 修复
- 修复了强制进程显示的问题 [#1447]
- 修复了与 GetClassName 相关的崩溃问题 [#1448]
- 修复了轻微的 UI 问题 [#1382]
- 修复了 UI 语言预设问题 [#1348]
- 修复了 SandMan UI 中的分组问题 [#1358]
- 修复了 EnableWin32kHooks 相关的问题 [#1458]

### 安装程序重新发布，修复如下：
- 修复了启动 Office 应用程序时的回归问题 [#1468]
...
- 在 Plus 安装程序中添加更多条目（当前翻译在 [Languages.iss] 文件中需要更新)
...
### 修复
- 修复了在请求的打开服务未运行时的 RPC 处理问题 [#1443]
- 修复了在 32 位应用程序中使用 NdrClientCall2 的钩子问题
- 修复了在使用 SandMan 时运行沙盒的起始目录问题 [#1436]
- 修复了从网络共享位置恢复的相关问题 [#1435]
...
- 添加了在 Windows 10 及更高版本上 Hook Win32k 系统调用的机制，这应该解决 Chromium 硬件加速的问题
  ...
  - 要便为所选盒子的所有进程强制使用 Win32k 钩子，请添加 "AlwaysUseWin32kHooks=program.exe,y" [#1261] [#1395]

### 修复
- 修复了 GetVersionExW 中的错误，导致 "OverrideOsBuild=..." 无法工作 [#605] [#1426]
- 修复了 INI 文件中使用某些 UTF-8 字符的问题
- 修复了与虚拟网络编辑器相关的隔离问题 [#1102]

## [1.0.2 / 5.55.2] - 2021-12-08

### 修复
- 修复了重载时恢复窗口未刷新计数的问题 [#1402]
- 修复了 1.0.0 中引入的打印问题 [#1397]
- 修复了与 CreateProcess 函数相关的问题 [#1408] [#1391]
...
- 修复了 1.0.0 版本引入的蓝屏死机问题 [#1389]
- 修复了在沙盒中运行 Chromium 浏览器时出现的多个 BITS 通知（由 isaak654 提供) [ca320ec] [#1081]
- 修复了 Run Menu 条目中可执行文件的选择（由 isaak654 提供) [#1379]

### 移除
- 移除了 Virtual Desktop Manager 模板（由 isaak654 提供) [d775807] [#1326]
...
- 重新设计资源访问路径匹配机制，以便选择性地对更具体的规则应用优先于较不具体的规则
  ...
  - 新增 "NormalFilePath=..." 以恢复特定路径上的默认 Sandybox 行为
...
### 修复
- 修复了 Sandybox 卸载过程中出现的 SBIE1401 通知 (by mpheath) [68fa37d]
- 修复了处理 FLT_FILE_NAME_INFORMATION 时的内存泄露 (by Therzok) [#1371]

## [0.9.8d / 5.53.3] - 2021-11-01

### 新增
- 如果用户希望在安装后启动 SandMan.exe，新增复选框 [#1318]
- 新增 Windows 10 虚拟桌面管理器的模板 [#1326]

### 更改
- "OpenClsid=..." 不再仅限于 CLSCTX_LOCAL_SERVER 执行上下文
  - 这允许在 COM 辅助服务中运行带有 CLSCTX_INPROC_SERVER 标志的对象
- 在跟踪视图中，现在可以一次选择多种类型
- 一些 Plus UI 项目已被设置为可翻译 (by gexgd0419) [#1320]
- 更改默认的 "终止所有被隔离进程" 键为 Shift+Pause (by isaak654) [#1337]

### 修复
- 修复了 SbieCtrl 与新的 INI 处理机制相关的 INI 写入问题 [#1331]
- 修复了跟踪日志过滤的问题
- 修复了 Plus 安装程序中的德国语言空格问题 (by mpheath) [#1333]
- 恢复了 Waterfox 钓鱼模板条目并进行了适当修复 (by APMichael) [#1334]

## [0.9.8c / 5.53.2] - 2021-10-24

### 新增
- 新增显式行以在卸载时删除空的 Shell 注册表项 (by mpheath) [3f661a8]

### 修复
- 修复了模板部分在编辑器中不显示的问题 [#1287]
- 修复了先前构建中自动删除箱子内容损坏的问题 [#1296] [#1324]
- 修复了在 "浏览内容" 窗口中的崩溃问题 [#1313]
- 修复了图标解析的问题 [#1310]
- 修复了状态栏中无效的 "无网络" 状态 [#1312]
- 修复了 Windows 资源管理器搜索框无法工作的问题 (by isaak654) [#1002]
- 修复了 Waterfox 钓鱼模板 (by Dyras) [#1309]
- 修复了 Plus 安装程序中的中文翻译文件问题 (by mpheath) [#1317]
- 修复了 Plus 安装程序上的自动运行注册表项路径 (by mpheath) [abd2d44]
...
- 在 Plus UI 中新增法语 (by clexanis) [#1155]

### 更改
- 网络流量跟踪现在正确记录到驱动程序日志，而不是内核调试日志
- Plus 安装程序将在安装后自动启动 SandMan.exe，以修复任务栏图标问题 [#post-3040211](https://www.wilderssecurity.com/threads/Sandybox-0-9-7-test-build.440906/page-4#post-3040211)
- 经典安装程序在更新时将显示许可协议 [#1187]

### 修复
- 修复了模板部分在编辑器中不显示的问题 [#1287]
- 修复了造成某些应用程序在任务栏中显示两个按钮组的应用程序 ID 问题 [#1101]
- 修复了 Plus UI 上最大 INI 值长度的问题 [#1293]
- 修复了最近引入的处理空 Sandboxie.ini 的问题 [#1292]
- 修复了 "SpecialImages" 模板的问题 (by Coverlin) [#1288] [#1289]
- 修复了箱子清空时的问题 [#1296]
- 修复了某些语言的问题 [#1304]
- 修复了挂载目录的问题 [#1302]
- 补充了 Qt 库的缺失翻译 [#1305]
- 修复了与 Windows 兼容性助手相关的问题 [#1265]
- 修复了特定进程图像设置的问题 [#1307]
...
- 在日志中新增 Sbie+ 版本 [#1277]
- 为 Plus 安装程序新增卸载清理额外文件的功能 (by mpheath) [#1235]
- 为 Plus 安装程序新增设置语言的功能 (by mpheath) [#1241]
- 新增与 SbieMsg.dll 的事件日志消息，用于 Plus 安装程序 (by mpheath)
- 现在保存分组展开状态
- 在跟踪标签中新增附加过滤器
- 在 Templates.ini 中新增一个新部分 [DefaultTemplates]，其中包含始终应用的强制模板 [0c9ecb0]

### 更改
- 重新设计并扩展 RPC 日志
- 重新引入 "UseRpcMgmtSetComTimeout=some.dll,n" 设置，当未指定 "RpcPortBinding" 条目时使用
  - 这允许独立于超时设置启用/禁用箱外 RPC 绑定
- 现在可以明确在每个图像名称基础上设置 "BoxNameTitle" 值 [#1190]

### 修复
- 修复了无法从被隔离的 Windows 资源管理器删除只读文件的问题 [#1237]
- 修复了 Plus UI 中错误的恢复目标 [#1274]
- 修复了在 0.9.7a 中引入的 SBIE2101 问题 [#1279]
- 修复了在箱子选择窗中排序的问题 [#1269]
- 修复了托盘刷新问题 [#1250]
- 修复了托盘活动显示 [#1221]
- 修复了恢复窗口在任务栏中不显示的问题 [#1195]
- 修复了暗主题预设不实时更新的问题 [#1270]
- 修复了 Microsoft Edge 关于 "FakeAdminRights=y" 的投诉问题 [#1271]
- 修复了在全局部分使用本地模板的问题 [#1212]
- 修复了 MinGW 中的 git.exe 冻结问题 [#1238]
- 修复了在黑暗模式下搜索高亮的问题
...
### 新增
- 在进程状态列中新增强制进程指示器 [#1174]
- 新增 "SbieTrace=y" 选项，以跟踪 Sandybox 进程与 Sandybox 核心组件之间的交互
- 在初始化空沙箱时，设置 MSI 调试键以生成 MSI 安装程序服务的调试输出
- 新增 "DisableComProxy=y" 以禁用通过服务的 COM 代理
- 新增 "ProcessLimit=..." 允许限制沙箱中进程的最大数量 [#1230]
- 新增缺失的 IPC 日志记录
...
- 改进了 Plus 安装程序的进度和状态消息 (by mpheath) [#1168]
...
### 更改
- 更新模板中的 BlockPort 规则，以符合新的 NetworkAccess 格式（by isaak654)[#1162]
- 立即恢复行为的默认设置现在是显示恢复窗口，而不是使用通知窗口 [#988]
- 新的运行对话框现在需要双击 [#1171]
- 重新设计了恢复窗口
...
- 修复了创建组菜单时的问题 [#1151]
- 修复了重命名时导致箱子失去组关联的问题
- 修复了与 Thunderbird 91+ 相关的问题 [#1156]
- 修复了文件处置处理的问题 [#1161]
- 修复了 Windows 11 22449.1000 的问题 [#1164]
- 修复了 SRWare Iron 模板 (by Dyras) [#1146]
- 修复了经典 UI 中标签定位的问题 (by isaak654) [#1088]
- 修复了仅将星号设置为路径时发生的旧问题 [#971]
...
- 在 Plus UI 中新增系统托盘选项以显示经典图标 [#963]

### 更改
- 将命令提示符图标和字符串从 "Terminal" 更改为 "Command Prompt" [#1135]
- 重新设计箱子菜单布局

### 修复
- 修复了与 Windows Server 2022（构建 20348)兼容性的问题 [#1143]
- 修复了创建快捷方式时的问题 [#1134]

### 安装程序于 2021-08-31 重新发布，修复了以下问题：
- 修复了 Plus 升级后 KmdUtil 警告 1061 (by mpheath) [#968] [#1139]
...
- 新增日志子菜单中的清除命令 [#391]
- 新增禁用进程终止提示的选项 [#514]
- 新增 "Options/InstantRecovery" 设置到 Sandboxie-Plus.ini，以使用恢复窗口替代通知弹出窗口 [#988]
- 新增重命名非空沙箱的能力 [#1100]
- 新增移除非空沙箱的能力
- 新增 SandMan UI 的文件浏览窗口以覆盖 SbieCtrl 的文件查看功能 [#578]
...
- 修复了组显示问题 [#1094]
- 修复了在已在沙箱中使用 "沙箱运行" 的文件时出现的问题 [#1099]
...
### 添加
- 可以与 Sandybox 链接一起使用“无沙箱运行”选项 [#614]

### 修复
- 修复了经典版本中的“在沙箱外运行”问题 [#614]
- 修复了打开模板时未加载编辑标签的问题 [#1054]
- 修复了“探索沙箱”问题 [#972]
- 修复了沙箱流程的起始目录问题 [#1071]
- 修复了语言自动检测问题 [#1018]
- 修复了多个同名文件的问题，通过始终显示扩展名 [#1041]
- 修复了 SandMan UI 中的多个程序分组问题 [#1054]
- 修复了“无磁盘”错误 [#966]
- 修复了使用 qMake 的 32 位版本的问题，-O2 选项导致 QSbieAPI.dll 崩溃 [#995]
- 修复了最近版本引入的 UserSettings 问题 [#1054]
...
### 修复
- 修复了 5.51.0 中引入的进程启动处理问题 [#1063]
- 修复了 5.51.0 中引入的快速恢复问题
- 修复了与 Intel 11 代和 AMD Ryzen 5XXX CPU 的 CET 硬件强制栈保护不兼容的问题 [#1067] [#1012]
...
- 添加对 Plus 安装程序的本地化支持（由 yfdyh000 和 mpheath 提供) [#923]
...
- 修复了 Inno Setup 脚本在 Sandybox 卸载时无法删除 shell 集成键的问题（由 mpheath 提供) [#1037]
...
### 修复
- 修复了“探索沙箱”命令的问题 [#972]
- 回滚了从使用 NtQueryKey 到 NtQueryObject 的切换，因为这似乎会破坏 Windows 10 版本 1803 及更早版本 [#984]
  - 此更改是为了修复 [#951]
...
### 修复
- 修复了之前版本中引入的自动删除激活的盒子的问题 [#986]

## [0.8.7 / 5.50.7] - 2021-07-10

### 添加
- 添加总是自动选择 DefaultBox 的选项 [#959]
...
### 更改
- 将默认的“终止所有沙箱进程”快捷键从 Ctrl+Pause 更改为 Ctrl+Alt+Pause [#974]
- Start.exe 不再链接未使用的 MFC 代码，将文件大小从 2.5 MB 降低到 250 KB 以下
- 更新了主 SandMan 和托盘图标 [#963]
- 改进了盒子树状视图

### 修复
- 向 KmdUtil.exe 添加了额外延迟和重试，以减轻卸载驱动程序时的问题 [#968]
- 修复了安装后 SbieCtrl 未正确启动的问题 [#969]
- 修复了“探索沙箱” shell 选项的问题 [#972]
- 修复了以提升权限运行 SandMan 时的问题 [#932]
...
### 添加
- 添加 LibreWolf 模板（由 Dyras 提供) [#929]
...
- 将 Librewolf.exe 添加到 Firefox 衍生程序列表中 [#927]
- 添加以沙箱方式运行 regedit 的菜单命令
- 向 SandMan UI 添加用于更新和新闻的新支持设置选项卡
- 向 Sbie 服务和 UI 添加代码完整性验证
- 为 Vivaldi Notes 添加模板（由 isaak654 提供) [#948]
...
- 盒子删除例程现在最多重试 10 次，见 [#954]
- 将服务使用的进程列表替换为速度更快的哈希映射实现
- 将服务使用的每个进程线程列表替换为速度更快的哈希映射实现

### 修复
- 修复了 SetServiceStatus 中的初始化错误（由 flamencist 提供) [#921]
- 修复了传统 UI 设置中的按钮位置（由 isaak654 提供) [#914]
- 修复了 SandMan UI 中缺少密码长度检查的问题 [#925]
- 修复了通过名称打开作业对象的问题
- 修复了重新打开作业对象句柄时缺少权限检查的问题（感谢 Diversenok)
- 修复了某些 Chromium 90+ 钩子影响派生浏览器中 PDF 显示的问题 [#930] [#817]
- 修复了与 SbieSvc 通信所用的 LPC 端口重新连接的问题
- 修复了设置问题 [#957]
- 修复了资源访问 COM 设置中的小型 UI 问题 [#958]
- 修复了 NtQueryKey 使用 NtQueryObject 的问题 [#951]
- 修复了在路径解析失败时的 key.c 崩溃
- 针对最上层模式问题添加了解决方法 [#873]
  - 通知窗口不仅在 5 秒内保持最上层状态
- 修复了 5.49.5 中引入的目录删除问题
- 修复了创建盒子副本时的问题

### 删除
- 删除 "BlockPassword=n" 的切换，因为它似乎没有效果 [#938]
  - 建议使用 "OpenSamEndpoint=y" 以允许 Windows 10 中的密码更改

## [0.8.2 / 5.50.2] - 2021-06-15

### 更改
- 按浏览器拆分反钓鱼规则（由 isaak654 提供) [#910]

### 修复
- 正确修复了与驱动程序验证和用户句柄有关的问题 [#906]
- 修复了 0.8.0 中引入的 CreateWindow 函数问题
- 修复了过时的 BoxDisplayOrder 条目保持的问题 [#900]
...
- 修复了 5.50.0 中引入的剪贴板问题 [#899]
- 修复了 Windows 7 32 位上 DcomLaunch 的问题 [#898]

## [0.8.0 / 5.50.0] - 2021-06-13

### 添加
- 通常，Sandybox 会对位于沙箱中的未排除图像应用 “Close...=!<program>,...” 指令
  - 添加 'AlwaysCloseForBoxed=n' 以禁用此行为，因为这可能并不总是想要的，并且它不会提供额外的安全性
- 向 SandMan UI 添加进程映像信息
- 在 Plus UI 中本地化模板类别 [#727]
- 添加 "DisableResourceMonitor=y" 以禁用对选定盒子的资源访问监视
- 添加选项以仅显示选定沙箱的跟踪条目 [#886]
- 添加 "UseVolumeSerialNumbers=y"，允许驱动器字母在 \drive\ 沙箱位置后跟卷序列号
  - 这有助于避免在使用相同字母的多个便携式驱动器上文件混合
  - 注意：此选项与经典 UI 的恢复功能不兼容，仅 SandMan UI 完全兼容
- 添加 "ForceRestart=PicoTorrent.exe" 到 PicoTorrent 模板以修复兼容性问题 [#720]
- 添加对 RPC 模板的本地化支持（由 isaak654 提供) [#736]

### 更改
- 便携式清理消息现在具有是/否/取消选项 [#874]
- 合并了 Proc_CreateProcessInternalW 和 Proc_CreateProcessInternalW_RS5 以删除重复代码
- ElevateCreateProcess 修复有时会被程序兼容性助手应用，现在默认不再模拟 [#858]
  - 使用 'ApplyElevateCreateProcessFix=y' 或 'ApplyElevateCreateProcessFix=program.exe,y' 启用它
- 仅当跟踪日志没有条目并且记录被停止时才禁用跟踪

### 修复
- 修复了新的全局钩子仿真机制和 WoW64 进程中的 APC 问题 [#780] [#779]
- 修复了与 BlockPort 选项有关的 IPv6 问题
- 修复了指定 "OpenWinClass=*" 时与 CheatEngine 有关的问题 [#786]
- 修复了 SbieDrv 中的内存损坏 [#838]
- 修复了 CreateProcess 调用中的进程提升崩溃问题 [#858]
- 修复了在内置管理员帐户下运行时的进程提升问题 [#3]
- 修复了模板预览重置未保存条目的盒子选项窗口的问题 [#621]

## [0.7.5 / 5.49.8] - 2021-06-05

### 添加
- 现在可以通过 "OpenClipboard=n" 禁用对沙箱的剪贴板访问 [#794]

### 更改
- 现在由于与 Unity 游戏的兼容性，OpenBluetooth 模板默认启用 [#799]
- 现在可以根据每个进程设置 "PreferExternalManifest=program.exe,y"

### 修复
- 修复了与最新 VS2019 更新的编译问题
- 修复了与 Vivaldi 浏览器的相关问题 [#821]
- 修复了 Plus UI 中一些盒子选项的相关问题 [#879]
- 修复了与基于 Chromium 的浏览器中的硬件加速有关的一些问题 [#795]
- “停止所有”命令现在优先发出 “KmdUtil scandll”，以解决 SbieDll.dll 使用时的问题
- 针对 Electron 应用程序的解决方法，通过在 GPU 渲染进程上强制附加命令行参数 [#547] [#310] [#215]
- 修复了软件兼容性选项卡无法始终正确显示模板名称的问题 [#774]

## [0.7.4 / 5.49.7] - 2021-04-11

### 添加
- 向 Plus UI 添加禁用文件迁移提示的选项，使用 PromptForFileMigration=n [#643]
- 为各种安全隔离功能添加 UI 选项
- 向 Plus UI 添加设置模板值的缺失功能
- 为 Popcorn-Time、Clementine Music Player、Strawberry Music Player、32 位 MPC-HC 添加模板（由 Dyras 提供) [#726] [#737]

### 更改
- 将 AutoRecover 和 Favourites 的默认设置对齐到 Plus 版本（感谢 isaak654) [#747]
- 电子邮件客户端和浏览器的列表现在集中于 Dll_GetImageType
- 将 Templates.ini 中的 localstore.rdf 引用替换为 xulstore.json（由 isaak654 提供) [#751]

### 修复
- 修复了与记录互联网阻塞相关的轻微问题
- 修复了定位于网络共享上的文件恢复问题 [#711]
- 修复了与 CallTrace 相关的 UI 问题 [#769]
- 修复了沙箱快捷方式创建时接收重复扩展名的问题 [#770]
- 修复了传统 UI 中标签错位的问题（感谢 isaak654) [#759]
- 修复了 SbieCtrl 中的分隔线问题（感谢 isaak654) [#761]
- 修复了 The Bat! 模板中损坏的路径（由 isaak654 提供) [#756]
- 修复了媒体播放器试图在盒子内写入不必要的媒体文件的问题（由 Dyras 提供) [#743] [#536]

## [0.7.3 / 5.49.5] - 2021-03-27

### 新增
- 新增 "UseSbieWndStation=y" 以在选定的进程中模拟 CreateDesktop，而不仅限于 Firefox 和 Chrome [#635]
- 新增选项以降低控制台主机进程的完整性，现在可以使用 "DropConHostIntegrity=y" [#678]
- 新增选项以轻松添加本地模板
- 新增新的 torrent 客户端和媒体播放器模板（由 Dyras 提供） [#719]

### 更改
- 重新设计窗口钩子机制以提高性能 [#697] [#519] [#662] [#69] [#109] [#193]
...
- 修复了兼容性设置未正确应用的问题
- 修复了在 0.7.1 中引入的自动删除问题 [#637]
- 修复了 NtSetInformationFile, FileDispositionInformation 导致 Opera 安装失败的问题
- 修复了在 0.7.2 构建中引入的 MacType 问题 [#676]
- 修复了当禁用窗口重命名选项时，全球沙箱化窗口钩子不工作的情况
- 修复了保存本地模板的问题
- 修复了使用 runas 启动在 Sandybox 监督之外创建的进程时的问题 [#688]
...
### 新增
- 新增修改报告的 Windows 版本的选项 "OverrideOsBuild=7601"，适用于 Windows 7 SP1 [#605]
- 追踪日志现在可以像树一样结构化，以进程作为根项，线程作为分支

### 更改
- SandboxieCrypto 现在始终迁移 CatRoot2 文件，以防止真实文件被锁定
- 大幅提高了追踪日志性能
- MSI 服务器现在可以在 "FakeAdminRights=y" 和 "DropAdminRights=y" 选项下运行 [#600]
...
### 修复
- 修复了在 5.48.0 中引入的一些安装程序的问题 [#595]
- 修复了 Plus UI 中的 "将用户添加到沙箱" 问题 [#597]
- 修复安全问题 ID-15：HostInjectDll 机制允许本地特权升级（感谢 hg421）
- 经典 UI 不再允许使用无效或保留的设备名称创建沙箱 [#649]
...
- 修复了针对 'AppXDeploymentClient.dll' 的免费下载管理器的问题，因此 RpcMgmtSetComTimeout=y 将默认用于此 [#573]
- 修复了未能阻止所有 WinRM 文件的问题，使用 "BlockWinRM=n" 可禁用此文件阻止
- 修复了 Sandybox Classic 在沙箱设置 -> 外观中保存任何选项时崩溃的问题（由 typpos 提供） [#586]
...
- LogAPI 库现在使用 Sandybox 的跟踪设施来记录事件，而不是使用其自己的管道服务器
...
- 修复了资源监视器按时间戳排序的问题
- 修复了无效的 Opera 书签路径（由 isaak654 提供） [#542]
- 修复安全问题 ID-12：驱动程序中的竞争条件允许获取对进程的提升权限句柄（感谢 typpos） [#549]
- 修复安全问题 ID-13："\\RPC Control\\samss lpc" 现在已被驱动程序过滤（感谢 hg421） [#553]
- 修复了从托盘还原主窗口的状态 [#288]
...
### 新增
- 新增对 Waterfox.exe、Palemoon.exe 和 Basilisk.exe Firefox 分支及 Brave.exe 的检测 [#468]
- 新增蓝牙 API 支持，IPC 端口可以使用 "OpenBluetooth=y" 打开 [#319] [#197]
  - 此功能是实验性的，未带 UTF-8 签名的文件也应被识别
  - "ByteOrderMark=yes" 已过时，Sandboxie.ini 现在始终带有 BOM/签名
- 旧版语言文件现在可以使用 UTF-8 编码
- 重新设计文件迁移行为，移除硬编码列表，支持使用模板 [#441]
- 修复了访问限制 UI 中的项目选择问题
- 修复了 SbieCtrl.exe 中的更新程序崩溃问题 [#450]
...
- 新增确认提示以终止所有命令
- 新增窗口标题到沙箱化进程信息 [#360]
- 新增基于 WinSpy 的沙箱化窗口查找器 [#351]
- 新增查看禁用沙箱的选项，并双击沙箱以启用它

### 更改
- "重置列" 现在根据内容调整大小，并且现在可以本地化 [#426]
- 模态窗口现在相对于父窗口居中 [#417]
- 改进了新沙箱窗口 [#417]

### 修复
- 修复了窗口模态性的问题 [#409]
- 修复了将主窗口设置为始终在最上层时的问题 [#417]
- 修复了与 Windows 10 Insider build 21286 的驱动程序问题
- 修复了快照对话框的问题 [#416]
- 修复了写入路径时已存在于快照中但不在外部的情况 [#415]
...
- 在 SandMan UI 中新增沙箱根文件夹的浏览按钮 [#382]
- 新增 Windows Explorer 信息消息
- 新增选项以始终将 SandMan UI 保持在最上层 [#352]
- 允许将文件拖放到 SandMan.exe 以沙箱运行 [#355]
- 在启动沙箱应用程序时启动 SandMan UI [#367]
- 恢复窗口的新布局
- Sbie 视图排序现在不区分大小写

### 修复
- 修复了主窗口隐藏时子窗口关闭终止应用程序的问题 [#349]
...
- 修复了 SBIE2303 中的钩子问题，涉及 Chrome、Edge 和可能的其他浏览器 [#68] [#166]
- 修复了执行快照操作时对运行进程的检查失败的问题
- 修复了一些框选项复选框未正确初始化的问题
- 修复了 SandMan 未连接到驱动程序时某些不可用选项未正确禁用的问题
- 修复了 MSI 安装程序无法在 Windows 20H2 上创建 "C:\Config.msi" 文件夹的问题 [#219]
- 增加了对通用列表命令的缺失本地化支持
...
### 新增
- 在 SandMan UI 中添加提示以选择链接是否在沙箱浏览器或非沙箱浏览器中打开 [#273]
- 增加更多恢复选项
...
### 更改
- 更改文档和更新 URL 为新的 Sandybox.com 域名
- 大幅改进设置脚本（感谢 mpheath）
- "OpenClsid=" 和 "ClosedClsid=" 现在支持指定程序或组名称
- 默认情况下，当以便携模式启动时，沙箱文件夹将位于 Sandybox 实例的父目录中

### 修复
- 新的 SandMan UI 中分组菜单无法完全使用 [#277]
- 修复了在 SandMan UI 中无法设置快速恢复的问题
- 修复了在 SandMan UI 中加载进程图标时导致的资源泄漏问题
- 修复了与 OpenToken 调试选项相关的问题
- 修复了 Chrome 在访问导致调用 "FindAppUriHandlersAsync" 的网站时崩溃的问题 [#198]
...
- 在模板视图中添加了文本过滤器
- 添加了新的兼容性模板：
  - Windows 10 核心 UI 组件：OpenIpcPath=\BaseNamedObjects\[CoreUI]-* 解决中文输入和表情符号的问题 [#120] [#88]
...
### 已更改
- AppUserModelID 字符串不再包含 Sandybox 版本字符串
...
### 修复
- 修复了因早期 Driver Verifier 修复引入的文件重命名错误 [#174] [#153]
- 修复了保存访问列表的问题
- 修复了 SandMan UI 中程序组解析的问题
- 修复了互联网访问限制选项的问题 [#177] [#185]
- 修复了在直接驱动器上删除沙箱的问题 [#139]
...
### 修复
- 修复了 SbieSvc 和其他组件中的线程句柄泄漏 [#144]
- msedge.exe 现在被归类为 Chromium 派生物
- 修复了与 Chrome 自身沙箱的 Chrome 86+ 兼容性错误 [#149]
...
- 新增 Sbie 日志消息的通知
- 新增沙盒预设子菜单，以快速更改一些设置
  - 启用/禁用 API 日志记录；LogAPI DLL 现已与 Sandybox 一起分发
  - 移除管理员权限
  - 阻止/允许互联网访问
  - 阻止/允许访问网络文件
- 在沙盒状态列中新增更多信息
...
- 修复了许多在启用驱动程序验证器时导致 SbieDrv.sys 出现蓝屏的问题 [#57]
...
- 新增 "SandboxieLogon=y"，使进程在 "Sandybox" 用户的 SID 下运行，而非匿名用户
  - 注意：必须启用全局选项 "AllowSandboxieLogon=y"，并首先手动创建 "Sandybox" 用户帐户，重载驱动程序，否则进程启动将失败
- 改进了驱动程序中关于进程创建错误的调试
...
- 在沙盒菜单中新增维护菜单，允许安装/卸载和启动/停止 Sandybox 驱动程序、服务
...
### 新增
- 创建了一个基于 Qt 的新用户界面，名为 SandMan（Sandybox 管理器）
- 资源监视器现在显示 PID
